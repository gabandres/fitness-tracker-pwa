/**
 * MIRROR of `packages/core/src/food-plausibility.ts` — keep the two identical.
 *
 * `functions/` is not a workspace and does not depend on `@macrolog/core`; it
 * hand-mirrors shared contracts, exactly as `food-search.ts` mirrors the wire
 * types. The copy below is verbatim apart from this header.
 *
 * **`test/food-plausibility-parity.spec.ts` imports BOTH copies and asserts
 * they return identical verdicts**, so a change to one that is not made to the
 * other fails the suite rather than silently letting the server and the clients
 * disagree about which foods are believable.
 */
