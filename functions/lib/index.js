...
exports.weeklyFirestoreBackup = exports.adminGetUserDetails = exports.adminExportData = exports.adminResetQuotas = exports.adminListCompedEmails = exports.adminSetCompedEmail = exports.adminOverridePlan = exports.adminResetPassword = exports.adminDeleteUser = exports.adminSuspendUser = exports.getAuditLogs = exports.getRecentActivity = exports.getPlatformStats = exports.listUsers = exports.stopImpersonation = exports.startImpersonation = exports.setAdminClaims = exports.bootstrapAdmin = exports.servePublicProfilePage = exports.ogImagePublicProfile = exports.sendWeeklyDigest = exports.onDailyWeightWriteMirrorPublicProfile = exports.onUserUpdateMirrorPublicProfile = exports.releasePublicSlug = exports.claimPublicSlug = exports.onSubscriptionWritten = exports.onDailyLogCreated = exports.sendWelcomeEmail = exports.publishUserCount = exports.statusPulse = exports.generateWeeklyReport = exports.sendDayThreeCoachPush = exports.sendDailyReminders = exports.deleteAccount = exports.exportUserData = exports.checkAccessStatus = exports.releaseConsultation = exports.reserveConsultation = exports.analyzePhoto = exports.logWebhook = void 0;
...
const resend_client_1 = require("./resend-client");
...
async function loadCompedEmails() {
    ...
    const emails = new Set((snap.data()?.compedEmails ?? [])
        .map((e) => e.trim().toLowerCase())
        .filter((e) => !!e));
    accessCache = { emails, fetchedAt: now };
    return emails;
}
async function isComped(email) {
    if (!email)
        return false;
...
function validateLogEntry(body) {
    ...
    if (b.exerciseCompleted !== undefined) {
        if (typeof b.exerciseCompleted !== "boolean")
            return "exerciseCompleted must be a boolean.";
    ...
    if (b.liftCompleted !== undefined) {
        if (typeof b.liftCompleted !== "boolean")
            return "liftCompleted must be a boolean.";
        if (b.liftCompleted)
    ...
    if (b.cardioCompleted !== undefined) {
        if (typeof b.cardioCompleted !== "boolean")
            return "cardioCompleted must be a boolean.";
        if (b.cardioCompleted)
            entry.exerciseCompleted = true;
    }
...
exports.logWebhook = (0, https_1.onRequest)({ cors: false }, async (req, res) => {
    // POST only
    if (req.method !== "POST") {
 ...
 */
async function enforceRateLimit(collectionName, uid, minIntervalMs, errorCode) {
    const ref = db.collection(collectionName).doc(uid);
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        const last = snap.data()?.lastCallAt?.toMillis() ?? 0;
        const now = Date.now();
        if (last && now - last < minIntervalMs) {
            throw new https_1.HttpsError("resource-exhausted", "Too many requests. Please slow down.", { code: errorCode, retryAfterMs: minIntervalMs - (now - last) });
        }
        tx.set(ref, { lastCallAt: firestore_1.Timestamp.now(), uid }, { merge: true });
    });
}
exports.analyzePhoto = (0, https_1.onCall)({ secrets: [geminiApiKey], maxInstances: 10 }, async (request) => {
    // Auth required (callable protocol auto-verifies ID token)
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
    const email = request.auth.token.email;
    const role = request.auth.token.stripeRole;
    ...
    await enforceRateLimit("photoRateLimit", uid, PHOTO_MIN_INTERVAL_MS, "PHOTO_RATE_LIMITED" /* ErrorCode.PHOTO_RATE_LIMITED */);
    ...
    if (!unlimited) {
        await db.runTransaction(async (tx) => {
            ...
            if (used >= effectiveLimit) {
                throw new https_1.HttpsError("resource-exhausted", `Daily limit of ${effectiveLimit} photo analyses reached. Resets at midnight UTC.`, { code: "PHOTO_QUOTA_EXCEEDED" /* ErrorCode.PHOTO_QUOTA_EXCEEDED */, limit: effectiveLimit });
            }
            photosUsedAfter = used + 1;
            tx.set(quotaRef, { count: photosUsedAfter, uid, date: today }, { merge: true });
        });
    }
    const { photoBase64, locale } = request.data;
    if (!photoBase64 || typeof photoBase64 !== "string") {
        throw new https_1.HttpsError("invalid-argument", "photoBase64 is required.", { code: "PHOTO_MISSING" /* ErrorCode.PHOTO_MISSING */ });
    }
    ...
    if (photoBase64.length > 20_000_000) {
        throw new https_1.HttpsError("invalid-argument", "Image too large after processing.", { code: "PHOTO_TOO_LARGE" /* ErrorCode.PHOTO_TOO_LARGE */ });
    }
...
- NaturalSlim Shake (1 scoop, ~28g powder, prepared with water): ~105 cal, 15g protein` + descriptionLangSuffix,
            ...
            config: {
                ...
                responseJsonSchema: {
                    ...
                    properties: {
                        reasoning: { type: "string", description: "Chain-of-thought: identify each item, estimate its volume/mass from visual cues " +
        ...
        if (calories == null) {
            throw new https_1.HttpsError("internal", "Gemini could not estimate calories from this image.", { code: "PHOTO_ESTIMATE_FAILED" /* ErrorCode.PHOTO_ESTIMATE_FAILED */ });
        }
        return {
            calories,
            protein: protein ?? 0,
        ...
        throw new https_1.HttpsError("internal", "Photo analysis failed.", { code: "PHOTO_ANALYZE_FAILED" /* ErrorCode.PHOTO_ANALYZE_FAILED */ });
    }
});
...
exports.reserveConsultation = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
    const email = request.auth.token.email;
    const role = request.auth.token.stripeRole;
    await enforceRateLimit("consultationRateLimit", uid, CONSULTATION_MIN_INTERVAL_MS, "CONSULTATION_RATE_LIMITED" /* ErrorCode.CONSULTATION_RATE_LIMITED */);
    ...
    await db.runTransaction(async (tx) => {
        ...
        const used = doc.exists ? doc.data().count : 0;
        if (used >= effectiveLimit) {
            throw new https_1.HttpsError("resource-exhausted", `Daily limit of ${effectiveLimit} consultations reached. Resets at midnight UTC.`, { code: "CONSULTATION_QUOTA_EXCEEDED" /* ErrorCode.CONSULTATION_QUOTA_EXCEEDED */, limit: effectiveLimit });
        }
 ...
 */
exports.releaseConsultation = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
 ...
 */
exports.checkAccessStatus = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        return {
    ...
    const uid = request.auth.uid;
    await enforceRateLimit("accessStatusRateLimit", uid, ACCESS_STATUS_MIN_INTERVAL_MS, "RATE_LIMITED" /* ErrorCode.RATE_LIMITED */);
    const email = request.auth.token.email;
    ...
    const photosUsed = photoSnap.exists ? photoSnap.data().count : 0;
    const consultUsed = consultSnap.exists ? consultSnap.data().count : 0;
    return {
        admin, comped,
 ...
 */
async function deleteSubcollection(parentPath, subPath) {
...
async function cancelStripeSubscriptions(uid) {
    try {
        ...
        const toCancel = snap.docs.filter((d) => ACTIVE.has(d.data()?.status));
        ...
        console.warn(`cancelStripeSubscriptions failed for uid=${uid} — Stripe customer may need manual cleanup in the dashboard.`, err);
    }
}
...
exports.exportUserData = (0, https_1.onCall)({ maxInstances: 5 }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in to export your data.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
    await enforceRateLimit("exportRateLimit", uid, EXPORT_DATA_MIN_INTERVAL_MS, "RATE_LIMITED" /* ErrorCode.RATE_LIMITED */);
    const userRef = db.doc(`users/${uid}`);
    const dumpCollection = async (name) => {
        const snap = await userRef.collection(name).get();
        return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    };
    const dumpQuota = async (coll) => {
        const snap = await db.collection(coll).where("uid", "==", uid).get();
        return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    };
    const [profileSnap, dailyLogs, presets, reports, dailyWeights, dailyWater, measurements, photoQuota, consultationQuota] = await Promise.all([
        userRef.get(),
    ...
    if (profileSnap.exists) {
        const { webhookApiKey: _wk, fcmToken: _ft, ...safe } = profileSnap.data();
        profile = safe;
    }
    const payload = {
        exportedAt: firestore_1.Timestamp.now().toDate().toISOString(),
        uid,
        profile,
        dailyLogs,
        presets,
    ...
    if (serialized.length > 9_000_000) {
        throw new https_1.HttpsError("resource-exhausted", "Your data is too large for an inline export. Contact support to receive a download link.", { code: "RATE_LIMITED" /* ErrorCode.RATE_LIMITED */, sizeBytes: serialized.length });
    }
    return payload;
});
exports.deleteAccount = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in to delete your account.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
    await enforceRateLimit("deleteRateLimit", uid, DELETE_ACCOUNT_MIN_INTERVAL_MS, "RATE_LIMITED" /* ErrorCode.RATE_LIMITED */);
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Account deletion failed. Please contact support.", { code: "ACCOUNT_DELETE_FAILED" /* ErrorCode.ACCOUNT_DELETE_FAILED */ });
    }
});
// ─── Feature 4: Daily Push Reminder ───────────────────────────────
exports.sendDailyReminders = (0, scheduler_1.onSchedule)({ schedule: "every 1 hours", timeZone: "UTC" }, async () => {
    ...
    await Promise.allSettled(usersSnap.docs.map(async (userDoc) => {
        ...
        const reminderHour = data.reminderHour ?? 20;
        const tzOffsetMin = data.timezoneOffsetMin ?? 0;
        ...
        const logsSnap = await db
            ...
            .doc(userDoc.id)
            .collection("dailyLogs")
            .where("timestamp", ">=", firestore_1.Timestamp.fromDate(startOfDayUtc))
            .limit(1)
            .get();
        if (!logsSnap.empty)
...
exports.sendDayThreeCoachPush = (0, scheduler_1.onSchedule)({ schedule: "every 1 hours", timeZone: "UTC" }, async () => {
    const messaging = (0, messaging_1.getMessaging)();
    const usersSnap = await db
        .collection("users")
        .where("fcmToken", "!=", null)
    ...
    await Promise.allSettled(usersSnap.docs.map(async (userDoc) => {
        ...
        if (oldestSnap.empty)
            return;
        const oldestTs = oldestSnap.docs[0].data().timestamp;
        if (!oldestTs)
            return;
...
exports.generateWeeklyReport = (0, https_1.onCall)({ secrets: [geminiApiKey], maxInstances: 5 }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be signed in.", { code: "UNAUTHENTICATED" /* ErrorCode.UNAUTHENTICATED */ });
    }
    const uid = request.auth.uid;
    ...
    if (!entitled) {
        throw new https_1.HttpsError("permission-denied", "Weekly report is a Pro feature.", { code: "REPORT_NOT_ENTITLED" /* ErrorCode.REPORT_NOT_ENTITLED */ });
    }
    const { systemInstruction, prompt } = (request.data ?? {});
    if (typeof systemInstruction !== "string" || systemInstruction.length === 0 ||
        systemInstruction.length > REPORT_SYSTEM_MAX_CHARS) {
        throw new https_1.HttpsError("invalid-argument", "systemInstruction missing or too large.", { code: "REPORT_PAYLOAD_INVALID" /* ErrorCode.REPORT_PAYLOAD_INVALID */ });
    }
    if (typeof prompt !== "string" || prompt.length === 0 ||
        prompt.length > REPORT_PROMPT_MAX_CHARS) {
        throw new https_1.HttpsError("invalid-argument", "prompt missing or too large.", { code: "REPORT_PAYLOAD_INVALID" /* ErrorCode.REPORT_PAYLOAD_INVALID */ });
    }
    ...
    if (!latestSnap.empty) {
        const generatedAt = latestSnap.docs[0].data().generatedAt;
        if (generatedAt && Date.now() - generatedAt.toMillis() < REPORT_MIN_INTERVAL_MS) {
            throw new https_1.HttpsError("resource-exhausted", "A weekly report was generated recently. Try again later.", { code: "REPORT_TOO_SOON" /* ErrorCode.REPORT_TOO_SOON */ });
        }
    }
    try {
        const client = new genai_1.GoogleGenAI({ apiKey: geminiApiKey.value() });
        ...
        if (!markdown) {
            throw new https_1.HttpsError("internal", "Empty response from Gemini.", { code: "REPORT_GENERATE_FAILED" /* ErrorCode.REPORT_GENERATE_FAILED */ });
        }
        ...
        throw new https_1.HttpsError("internal", "Report generation failed.", { code: "REPORT_GENERATE_FAILED" /* ErrorCode.REPORT_GENERATE_FAILED */ });
    }
});
...
exports.statusPulse = (0, scheduler_1.onSchedule)({ schedule: "every 5 minutes", timeZone: "UTC" }, async () => {
    await db.doc("status/heartbeat").set({
        lastPulseAt: firestore_1.Timestamp.now(),
    });
});
...
exports.publishUserCount = (0, scheduler_1.onSchedule)({ schedule: "every 60 minutes", timeZone: "UTC" }, async () => {
...
exports.sendWelcomeEmail = (0, firestore_2.onDocumentUpdated)({
    document: "users/{uid}",
    secrets: [resend_client_1.resendApiKey],
}, async (event) => {
    const before = event.data?.before?.data();
    const after = event.data?.after?.data();
    if (!before || !after)
        return;
    const flippedToCompleted = before.profileCompleted !== true && after.profileCompleted === true;
    ...
    const preferredLocale = after.preferredLocale;
    const locale = preferredLocale === "es-PR" ? "es-PR" : "en";
    const displayName = after.displayName ??
        (await (0, auth_1.getAuth)().getUser(uid).then((u) => u.displayName).catch(() => null));
    const { subject, html } = (0, email_templates_1.welcomeEmail)({ locale, displayName });
    ...
    try {
        const resend = (0, resend_client_1.getResend)();
        const { error } = await resend.emails.send({
            ...(0, resend_client_1.baseSendOptions)(),
            to: email,
            subject,
            html,
        });
        ...
        await db.doc(`users/${uid}`).set({ welcomeEmailSentAt: firestore_1.Timestamp.now() }, { merge: true });
...
exports.onDailyLogCreated = (0, firestore_2.onDocumentCreated)("users/{uid}/dailyLogs/{logId}", async (event) => {
    const uid = event.params.uid;
    const profileRef = db.doc(`users/${uid}`);
    const snap = await profileRef.get();
    if (!snap.exists)
        return;
    if (snap.data()?.["firstEntryAt"] != null)
        return;
    await profileRef.update({ firstEntryAt: firestore_1.Timestamp.now() });
});
...
exports.onSubscriptionWritten = (0, firestore_2.onDocumentWritten)("users/{uid}/subscriptions/{subId}", async (event) => {
    const after = event.data?.after?.data();
    if (!after)
        return;
    ...
    if (referee["referralRewardGrantedAt"] != null)
        return; // already granted
    const referrerUid = referee["referredBy"];
    ...
    if (!referrerSnap.exists) {
        ...
        await refereeRef.update({
            referralRewardGrantedAt: firestore_1.Timestamp.now(),
            compedUntil: firestore_1.Timestamp.fromMillis(Date.now() + REFERRAL_REWARD_MS),
        });
    ...
    const grant = (current) => {
        ...
        return firestore_1.Timestamp.fromMillis(base + REFERRAL_REWARD_MS);
    };
    await Promise.all([
        refereeRef.update({
            referralRewardGrantedAt: firestore_1.Timestamp.now(),
            compedUntil: grant(referee["compedUntil"]),
        }),
        referrerRef.update({
            compedUntil: grant(referrerSnap.data()?.["compedUntil"]),
        }),
    ]);
    console.log(`referral: granted +${REFERRAL_REWARD_DAYS}d to referrer=${referrerUid} + referee=${refereeUid}`);
});
var public_profile_1 = require("./public-profile");
Object.defineProperty(exports, "claimPublicSlug", { enumerable: true, get: function () { return public_profile_1.claimPublicSlug; } });
Object.defineProperty(exports, "releasePublicSlug", { enumerable: true, get: function () { return public_profile_1.releasePublicSlug; } });
Object.defineProperty(exports, "onUserUpdateMirrorPublicProfile", { enumerable: true, get: function () { return public_profile_1.onUserUpdateMirrorPublicProfile; } });
Object.defineProperty(exports, "onDailyWeightWriteMirrorPublicProfile", { enumerable: true, get: function () { return public_profile_1.onDailyWeightWriteMirrorPublicProfile; } });
var weekly_digest_1 = require("./weekly-digest");
Object.defineProperty(exports, "sendWeeklyDigest", { enumerable: true, get: function () { return weekly_digest_1.sendWeeklyDigest; } });
var og_image_1 = require("./og-image");
Object.defineProperty(exports, "ogImagePublicProfile", { enumerable: true, get: function () { return og_image_1.ogImagePublicProfile; } });
Object.defineProperty(exports, "servePublicProfilePage", { enumerable: true, get: function () { return og_image_1.servePublicProfilePage; } });
var admin_claims_1 = require("./admin-claims");
Object.defineProperty(exports, "bootstrapAdmin", { enumerable: true, get: function () { return admin_claims_1.bootstrapAdmin; } });
Object.defineProperty(exports, "setAdminClaims", { enumerable: true, get: function () { return admin_claims_1.setAdminClaims; } });
var impersonation_1 = require("./impersonation");
Object.defineProperty(exports, "startImpersonation", { enumerable: true, get: function () { return impersonation_1.startImpersonation; } });
Object.defineProperty(exports, "stopImpersonation", { enumerable: true, get: function () { return impersonation_1.stopImpersonation; } });
var admin_ops_1 = require("./admin-ops");
Object.defineProperty(exports, "listUsers", { enumerable: true, get: function () { return admin_ops_1.listUsers; } });
Object.defineProperty(exports, "getPlatformStats", { enumerable: true, get: function () { return admin_ops_1.getPlatformStats; } });
Object.defineProperty(exports, "getRecentActivity", { enumerable: true, get: function () { return admin_ops_1.getRecentActivity; } });
Object.defineProperty(exports, "getAuditLogs", { enumerable: true, get: function () { return admin_ops_1.getAuditLogs; } });
Object.defineProperty(exports, "adminSuspendUser", { enumerable: true, get: function () { return admin_ops_1.adminSuspendUser; } });
Object.defineProperty(exports, "adminDeleteUser", { enumerable: true, get: function () { return admin_ops_1.adminDeleteUser; } });
Object.defineProperty(exports, "adminResetPassword", { enumerable: true, get: function () { return admin_ops_1.adminResetPassword; } });
Object.defineProperty(exports, "adminOverridePlan", { enumerable: true, get: function () { return admin_ops_1.adminOverridePlan; } });
Object.defineProperty(exports, "adminSetCompedEmail", { enumerable: true, get: function () { return admin_ops_1.adminSetCompedEmail; } });
Object.defineProperty(exports, "adminListCompedEmails", { enumerable: true, get: function () { return admin_ops_1.adminListCompedEmails; } });
Object.defineProperty(exports, "adminResetQuotas", { enumerable: true, get: function () { return admin_ops_1.adminResetQuotas; } });
Object.defineProperty(exports, "adminExportData", { enumerable: true, get: function () { return admin_ops_1.adminExportData; } });
Object.defineProperty(exports, "adminGetUserDetails", { enumerable: true, get: function () { return admin_ops_1.adminGetUserDetails; } });
exports.weeklyFirestoreBackup = (0, scheduler_1.onSchedule)({ schedule: "0 6 * * 0", timeZone: "UTC" }, async () => {
    if (!BACKUP_BUCKET) {
    ...
    try {
        ...
        console.log(`weeklyFirestoreBackup: export started → ${outputUri}. operation=${operation.name}`);
    }
    catch (err) {
        // Typical first-run failure: bucket doesn't exist yet. Log and
        // continue — this function is opt-in infrastructure.
        console.error(`weeklyFirestoreBackup: export failed. Ensure ${BACKUP_BUCKET} exists and ` +
            "Firebase service account has roles/datastore.importExportAdmin. Error:", err);
    }
});
