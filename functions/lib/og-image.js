...
exports.ogImagePublicProfile = exports.servePublicProfilePage = void 0;
...
function renderSvg(p) {
    ...
    const subtitle = p.startedAt
        ...
        : "Tracked with Macro Log";
    // Stat boxes: 3 columns, big serif numbers, small mono labels.
    const stat = (x, label, value, color = INK) => `
...
function injectMeta(html, params) {
    const tags = [
...
exports.servePublicProfilePage = (0, https_1.onRequest)({ cors: true, maxInstances: 5 }, async (req, res) => {
    ...
    const displayName = data["displayName"] || "Macro Log user";
    const totalChange = data["totalChange"] ?? null;
    const title = totalChange != null && totalChange < 0
        ? `${displayName} lost ${Math.abs(totalChange).toFixed(1)} lb · Macro Log`
        : totalChange != null && totalChange > 0
            ? `${displayName} gained ${Math.abs(totalChange).toFixed(1)} lb · Macro Log`
            : `${displayName}'s progress · Macro Log`;
    const description = "Tracked with Macro Log — a quiet, private calorie and protein log. Free TDEE calculator and weight tracking.";
...
exports.ogImagePublicProfile = (0, https_1.onRequest)({ cors: true, maxInstances: 5 }, async (req, res) => {
    ...
    try {
        const png = new resvg_js_1.Resvg(svg, {
