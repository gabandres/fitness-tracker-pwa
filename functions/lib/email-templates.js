...
function brandButton(text, href) {
...
function welcomeEmailEn(firstName) {
    ...
    body += paragraph("Macro Log is a quiet, private calorie and protein log. No ads, no streaks that shame you, no red-and-green scoreboards — just a calm place to record what you eat so the math can help you.");
    ...
    body += caption("Privacy: your logs are yours. We don't sell data, we don't run ads, and the AI coach only sees summaries of your data in-flight — never your email or name. Full policy at <a href=\"https://macrolog.web.app/privacy\" style=\"color:" + BLOOD + ";\">macrolog.web.app/privacy</a>.");
...
function weeklyDigestEmail(params) {
    ...
    const fmt = (n, suffix) => n == null ? noDataLabel : `${Math.round(n)}${suffix}`;
    const fmtDelta = (n) => {
        if (n == null)
            return noDataLabel;
        const sign = n >= 0 ? "+" : "−";
...
function welcomeEmailEs(firstName) {
    const subject = "Bienvenido a Macro Log.";
    const heading = "Listo.";
    const salutation = firstName ? `Hola <strong>${escapeHtml(firstName)}</strong>,` : "Hola,";
    let body = paragraph(salutation);
    body += paragraph("Macro Log es una bitácora privada y silenciosa de calorías y proteína. Sin anuncios, sin rachas que te avergüencen, sin marcadores rojo-verde — solo un lugar tranquilo para anotar lo que comes, y dejar que las matemáticas te ayuden.");
    ...
    body += caption("Privacidad: tu bitácora es tuya. No vendemos datos, no mostramos anuncios, y el coach IA solo ve resúmenes de tus datos en tránsito — nunca tu correo o nombre. Política completa en <a href=\"https://macrolog.web.app/privacy\" style=\"color:" + BLOOD + ";\">macrolog.web.app/privacy</a>.");
    return { subject, html: layout(heading, body) };
}
