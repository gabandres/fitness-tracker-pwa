...
function isSundayLocalHour(timezoneOffsetMin, nowMs) {
...
async function computeStatsForUser(uid, nowMs) {
    const db = (0, firestore_1.getFirestore)();
    const cutoff = firestore_1.Timestamp.fromMillis(nowMs - WEEK_MS);
    ...
    const weights = dwSnap.docs.map((d) => d.data())
        .filter((d) => d.date instanceof firestore_1.Timestamp && typeof d.weight === "number");
    const within = weights.filter((w) => w.date.toMillis() >= nowMs - WEEK_MS);
    let weightDeltaLbs = null;
    if (within.length >= 2) {
...
exports.sendWeeklyDigest = (0, scheduler_1.onSchedule)({
    ...
    for (const doc of optInSnap.docs) {
        const uid = doc.id;
        const data = doc.data();
        const lastSent = data["lastWeeklyDigestSentAt"];
        if (lastSent && nowMs - lastSent.toMillis() < SEND_GUARD_MS) {
            skipped++;
            continue;
        }
        const tzOffset = data["timezoneOffsetMin"];
        ...
        const claimed = await db.runTransaction(async (tx) => {
            ...
            if (!snap.exists)
                return false;
            const last = snap.data()?.["lastWeeklyDigestSentAt"];
            if (last && nowMs - last.toMillis() < SEND_GUARD_MS)
                return false;
            tx.set(userRef, { lastWeeklyDigestSentAt: firestore_1.Timestamp.fromMillis(nowMs) }, { merge: true });
            return true;
        });
        if (!claimed) {
            skipped++;
        ...
        const locale = data["preferredLocale"] === "es-PR" ? "es-PR" : "en";
        const displayName = data["displayName"]
            || (await (0, auth_1.getAuth)().getUser(uid).then((u) => u.displayName).catch(() => null));
        const { subject, html } = (0, email_templates_1.weeklyDigestEmail)({ locale, displayName, ...stats });
        try {
