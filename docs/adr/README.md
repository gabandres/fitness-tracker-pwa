...
| 0041 | [Train separates the logger from the coach](0041-train-ux-overhaul-logger-vs-coach.md) | **accepted 2026-09-16** — the progression engine's vocabulary had become the whole tab's default. The home screen now answers "what today" (`nextTemplateUp`, least-recently-performed), the set row carries PREVIOUS per row, one action stays on each exercise card and the rest go behind `⋯`, and the template editor asks for the structure FIRST and scaffolds from it — **reversing ADR-0040's UI ordering while leaving its model untouched**. Also closes two real defects: `EXERCISE_LIBRARY` was unreachable from every picker, and `muscles` was write-once, making `weeklyClusterAudit`'s `unattributed` line unfixable. Engine untouched; JS-only |
| 0042 | [Android's build host returns to `ignia-mac`, and the route is raw Gradle](0042-android-build-host-returns-to-the-mac.md) | **accepted 2026-09-17 — capability verified, cutover NOT executed.** The 2026-08-17 "permanently on Windows" rested on disk pressure that no longer exists (43 GB → 92 GB free after reclaiming 54 GB of waste). Route B (raw Gradle + `patch-android-release.mjs` + the verifier, which is now non-optional) measured green on the Mac: **15m04s, 62.6 MB AAB, signer byte-identical to `dev.keystore`, channel injected, verifier 10/10**; the emulator boots headless in 30 s and Maestro drives it with no phone. Route A (`eas build --local`) also verified and deliberately NOT adopted. **Android OTAs still publish from Windows until one Mac-built binary reaches Play** — the hashes cannot converge. Seven traps recorded, chiefly **T1: `expo prebuild` without `--no-install` runs `npm install` and shuts BOTH OTA channels; `npm ci` restores them exactly** — and the `build-android` skill's own Step 3 carries that bug |
