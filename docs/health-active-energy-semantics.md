...
8. **Confirm — not a blocker.** Does a genuinely-worn, well-instrumented device ever *frequently* produce legitimate active-energy days that round to `0`? [#29](https://github.com/gabandres/fitness-tracker-pwa/issues/29) rules a stored `activeKcal === 0` an **absence** (excluded from both the trailing mean and `usableDays`), on the physiological premise that any real movement registers active energy and a worn device rarely rounds a full day to 0. The rule is deliberately robust either way — it also normalizes Android's empty-bucket-returns-explicit-0 divergence (§below) — so this observation does **not** gate the decision. It is the single finding that would *reopen* it: if worn devices routinely surface rounds-to-0 real days, exclude-0 would be discarding low-activity signal and starving the `>= 21` gate. *Settled by:* the same import dev build (item 7) — log the per-day aggregate distribution across a fully-worn week and check for legitimate rounds-to-0 days. Secondary, also non-blocking: confirm whether Health Connect omits the aggregate vs returns explicit `0` for an empty bucket (the exclude-0 rule makes both harmless).

Items 1–7 require an EAS dev build on a physical device before they can be settled; none can be settled from documentation, and none from production data, which does not exist yet. Item 8 is already settled by decision (#29) and only *confirmed* by a device — it blocks nothing.
