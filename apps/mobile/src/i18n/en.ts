...
export const en = {
  ...
  'onboarding.remindersQ': 'Want a nudge at meal times?',
  'onboarding.remindersBody': 'Ignia can remind you on this phone, quietly. No account needed for it, and nothing leaves the device.',
  'onboarding.remindersLunch': 'Lunch · {t}',
  'onboarding.remindersDinner': 'Dinner · {t}',
  'onboarding.remindersStreak': 'Only if a day is about to go unlogged · {t}',
  'onboarding.remindersNote': 'Change the times or switch them off any time in Settings.',
  'onboarding.remindersOn': 'Turn on reminders',
  'onboarding.remindersNotNow': 'Not now',
  'onboarding.firstLogQ': 'Log what you last ate',
  'onboarding.firstLogBody': 'One entry is enough to see your plan working. It takes under a minute.',
  'onboarding.firstLogSearch': 'Search a food',
  'onboarding.firstLogPhoto': 'Snap a photo of the plate',
  'onboarding.firstLogType': 'Or just type it in',
  'onboarding.firstLogNote': "Nothing has to be exact. A rough entry beats an empty day.",
  'onboarding.firstLogCta': 'Log my last meal',
  'onboarding.firstLogLater': "I'll do it later",
  ...
  // ── Progression engine (layers 1-6; @macrolog/core progression-engine.ts) ──
  // The action is in caps because it is the one word read between sets. Every
  // number in a reason comes off the Recommendation object — nothing is
  // rounded or invented here.
  'train.rec.nextSession': 'Next session',
  'train.rec.hide': 'Hide',
  'train.rec.action.addLoad': 'ADD LOAD',
  'train.rec.action.reduceAssist': 'REDUCE ASSISTANCE',
  'train.rec.action.hold': 'HOLD AND BUILD',
  'train.rec.action.buildReps': 'BUILD REPS',
  'train.rec.action.repeat': 'REPEAT — read invalid',
  'train.rec.action.calibrate': 'CALIBRATING',
  'train.rec.reason.noHistory': 'No session logged yet — the first one is a calibration, not a read.',
  'train.rec.reason.atTarget': '{reps} reps @ RIR {rir} at {load} — the add-load mark is {at}.',
  'train.rec.reason.atTargetNoRir': '{reps} reps at {load} — the add-load mark is {at}.',
  'train.rec.reason.bodyweightAdd': '{reps} reps at bodyweight — add a plate or a band.',
  'train.rec.reason.overBand': '{reps} reps — over the add-load mark of {at}, under-loaded.',
  'train.rec.reason.belowBand': '{reps} reps last session; hold {lo}-{hi}, add load at {at}.',
  'train.rec.reason.belowBandCluster': 'C{group} gave {reps} reps last session; both clusters need {at} to add load.',
  'train.rec.reason.underBand': '{reps} reps last session — under the {lo}-{hi} hold band. Build back to {goal} at {load}.',
  'train.rec.reason.underBandCluster': 'C{group} gave {reps} reps — under the {lo}-{hi} hold band. Build C{group} back to {goal} at {load}.',
  ...
  'train.structure.straight': 'straight sets',
  'train.structure.myoreps': 'myo-reps',
  'train.structure.restPause': 'rest-pause',
  'train.structure.cluster': 'cluster sets',
  ...
  'train.rec.calibrating': 'Calibrating — {n} of {needed} valid sessions logged.',
  'train.rec.warn.failureOnRir1': 'Logged at failure. This lift is set to leave 1 rep in reserve.',
  'train.rec.settings': 'Lift settings',
  'train.rec.reason.jumpTooBig': 'Next available load is {next} (+{pct}%). Build to {reps} reps at {load} first.',
  'train.rec.clusterPrefix': 'C{group}: ',
  'train.rec.invalid.firstMiniTooMany': 'First mini was {n} reps; above 5 means the activation was not close enough to failure.',
  'train.rec.invalid.firstMiniTooFew': 'First mini was {n} reps; below 2 means the activation went too close to failure.',
  'train.rec.invalid.miniExceeds': 'A mini-set ({n}) out-repped the activation ({reps}).',
  'train.rec.invalid.loadChanged': 'Load changed between clusters — one load per exercise.',
  'train.rec.invalid.repsMissing': 'No rep count on the activation.',
  'train.rec.invalid.rirMissing': 'No RIR on the activation — log it on the activation set.',
  'train.rec.invalid.rirTooEasy': 'Activation at RIR {rir} — too far from failure.',
  'train.rec.invalid.minisMissing': 'No mini-sets logged after the activation.',
  'train.rec.invalid.notClustered': 'Logged as straight sets, not as a cluster.',
  'train.rec.last': 'Last: {reps} reps @ RIR {rir}',
  'train.rec.lastNoRir': 'Last: {reps} reps',
  'train.rec.lastCluster': 'C{group} {reps} @ RIR {rir}',
  'train.rec.lastClusterNoRir': 'C{group} {reps}',
  'train.rec.stall': 'STALLED — {n} sessions at {load}.',
  'train.rec.stallInvalid': '{n} of {total} reads were invalid — that is the cause.',
  'train.rec.stallEasy': 'First mini above 5 in {n} of {total} — the activation was too easy.',
  'train.rec.stallBlocked': 'C{group} blocked every time.',
  'train.rec.fix.shortenMiniRest': '{n}. Shorten mini rest to 5-10 s.',
  'train.rec.fix.reduceBlockingCluster': '{n}. Drop C{group} by one increment.',
  'train.rec.fix.reduceLoad': '{n}. Reduce load 10% and rebuild.',
  'train.lift.title': 'Lift settings',
  'train.lift.effort': 'Effort standard — activation set',
  'train.lift.effortFailure': 'To failure (RIR 0)',
  'train.lift.effortRir1': '1 rep in reserve',
  'train.lift.effortHint': 'Failure is the standard. Leave 1 in reserve only where failure under load conflicts with a restriction.',
  'train.lift.band': 'Rep band',
  'train.lift.bandDerived': 'Automatic — from your last {n} valid sessions at {load}: hold {lo}-{hi}, add load at {at}.',
  'train.lift.bandCalibrating': 'Automatic — calibrating, {n} of {needed} valid sessions at this load.',
  'train.lift.bandOverride': 'Override — add load at {at} reps (hold {lo}-{hi}). Automatic is off.',
  'train.lift.bandField': 'Add load at',
  'train.lift.bandReps': 'reps',
  'train.lift.bandHint': 'Leave empty for automatic. The hold band is the two reps under the mark.',
  'train.lift.bandClear': 'Back to automatic',
  'train.lift.save': 'Save',
  // Weekly cluster audit (layer 5). CLUSTERS, not sets: one myo-reps cluster
  // is one rest-pause set, and the rest-pause range is 2-6 per muscle per week.
  'train.audit.title': 'Clusters this week',
  'train.audit.hint': 'One cluster = one rest-pause set. Range: 2-6 per muscle per week.',
  'train.audit.below': 'below range',
  'train.audit.inRange': 'in range',
  'train.audit.above': 'above range — check recovery',
  'train.audit.unattributed': 'No muscle group set: {names}',
  'train.muscle.chest': 'Chest',
  'train.muscle.back': 'Back',
  'train.muscle.shoulders': 'Shoulders',
  'train.muscle.biceps': 'Biceps',
  'train.muscle.triceps': 'Triceps',
  'train.muscle.quads': 'Quads',
  'train.muscle.hamstrings': 'Hamstrings',
  'train.muscle.glutes': 'Glutes',
  'train.muscle.calves': 'Calves',
  'train.muscle.core': 'Core',
  'train.muscle.forearms': 'Forearms',
  ...
  'settings.redoSetup': 'Redo setup',
  'settings.redoSetupSub': 'Asks every setup question again',
  ...
  'reminder.lapsed7Body': 'A week away changes nothing. Log one thing to pick up where you left off.',
  ...
  'targets.errNumber': 'Enter a number.',
  'targets.errProteinMin': 'Protein target must be at least {n} g.',
  'targets.errProteinMax': 'Protein target cannot be above {n} g.',
