...
export const esPR: Record<I18nKey, string> = {
  ...
  'scan.hint': 'Apunta a tu plato — Ignia estima las calorías y macros en un toque.',
  ...
  'scan.estimateHint':
    'Los alimentos marcados como "estimado" no están en la base de datos, así que esos números son un estimado de Ignia. Ajústalos si sabes mejor.',
  ...
  'scan.errProvider':
    'El escaneo no está disponible ahora mismo — es cosa nuestra, no de tu foto. Intenta de nuevo más tarde.',
  ...
  'onboarding.remindersQ': '¿Quieres un recordatorio a la hora de comer?',
  'onboarding.remindersBody': 'Ignia puede recordarte en este teléfono, sin ruido. No hace falta nada más y nada sale del dispositivo.',
  'onboarding.remindersLunch': 'Almuerzo · {t}',
  'onboarding.remindersDinner': 'Cena · {t}',
  'onboarding.remindersStreak': 'Solo si un día está por quedar sin registro · {t}',
  'onboarding.remindersNote': 'Cambia las horas o apágalos cuando quieras en Ajustes.',
  'onboarding.remindersOn': 'Activar recordatorios',
  'onboarding.remindersNotNow': 'Ahora no',
  'onboarding.firstLogQ': 'Registra lo último que comiste',
  'onboarding.firstLogBody': 'Con una entrada basta para ver tu plan en acción. Toma menos de un minuto.',
  'onboarding.firstLogSearch': 'Busca un alimento',
  'onboarding.firstLogPhoto': 'Toma una foto del plato',
  'onboarding.firstLogType': 'O simplemente escríbelo',
  'onboarding.firstLogNote': 'No tiene que ser exacto. Una entrada aproximada vale más que un día vacío.',
  'onboarding.firstLogCta': 'Registrar mi última comida',
  'onboarding.firstLogLater': 'Lo hago después',
  ...
  'history.legendLogged': 'con registro',
  'history.legendWeighed': 'pesaje',
  'common.back': 'Atrás',
  ...
  'entry.savePresetShort': 'Guardar preajuste',
  ...
  'entry.scanBarcode': 'Escanea un código de barras',
  'entry.importRecipe': 'Importa desde un enlace',
  ...
  'mealText.assumed': 'Esa porción no está en la base de datos — la cantidad de arriba es un estimado. Edítala.',
  ...
  'fast.runningTitle': 'Hora de inicio del ayuno',
  'fast.length': '{h}h {m}m',
  'fast.runningFor': 'ayunando {h}h {m}m',
  'fast.started': 'Empezó',
  'fast.ended': 'Terminó',
  'fast.runningNote': 'Todavía sigue — termínalo en Hoy cuando lo rompas.',
  'fast.hour': 'Hora',
  'fast.minute': 'Minuto',
  'fast.dayEarlier': 'Día anterior',
  'fast.dayLater': 'Día siguiente',
  'fast.errOrder': 'El final tiene que ser después del inicio.',
  'fast.errFuture': 'Eso está en el futuro.',
  'fast.errTooLong': 'Más de {n} días es un cronómetro corriendo, no un ayuno.',
  'fast.errOverlap': 'Se solapa con tu ayuno del {when}.',
  'fast.errOverlapMany': 'Se solapa con {n} de tus otros ayunos.',
  ...
  'fast.add': 'Añadir un ayuno',
  'fast.none': 'Ningún ayuno terminó este día.',
  'fast.byHand': 'a mano',
  'fast.range': '{from} – {to}',
  'fast.editHint': 'Editar este ayuno',
  ...
  // ── train ──
  'train.start': 'Empezar entrenamiento',
  'train.history': 'Historial',
  'train.noWorkouts': 'Aún no hay entrenamientos. Empieza uno arriba.',
  ...
  'train.finishTitle': 'Terminar entrenamiento',
  'train.finishHint': 'Opcional — se registra en Cuerpo + Hoy.',
  'train.bodyweight': 'Peso corporal ({unit})',
  'train.sleepH': 'Sueño (h)',
  'train.complete': 'Completar entrenamiento',
  'train.templates': 'Plantillas',
  'train.newTemplate': '+ Nueva',
  'train.noTemplates': 'Aún no hay plantillas. Crea una para empezar un entrenamiento en un toque.',
  ...
  'train.invalidRirEasy': 'La activación fue RIR 4+ — sin cambio de carga.',
  'train.invalidRirMissing': 'Sin RIR en la activación — no se puede evaluar.',
  'train.invalidNotClustered': 'Anotado como series rectas, no como clúster.',
  'train.invalidHeading': 'No evaluable esta sesión',
  'train.invalidHint': 'Se guardó igual — solo no pueden cambiar la carga.',
  'train.rec.nextSession': 'Próxima sesión',
  'train.rec.hide': 'Ocultar',
  'train.rec.action.addLoad': 'SUBIR CARGA',
  'train.rec.action.reduceAssist': 'REDUCIR ASISTENCIA',
  'train.rec.action.hold': 'MANTENER Y SUMAR REPS',
  'train.rec.action.buildReps': 'SUMAR REPS',
  'train.rec.action.repeat': 'REPETIR — lectura inválida',
  'train.rec.action.calibrate': 'CALIBRANDO',
  'train.rec.reason.noHistory': 'Aún sin sesión anotada — la primera es calibración, no una lectura.',
  'train.rec.reason.atTarget': '{reps} reps @ RIR {rir} con {load} — la marca para subir carga es {at}.',
  'train.rec.reason.atTargetNoRir': '{reps} reps con {load} — la marca para subir carga es {at}.',
  'train.rec.reason.bodyweightAdd': '{reps} reps a peso corporal — añade un disco o una banda.',
  'train.rec.reason.overBand': '{reps} reps — por encima de la marca de {at}, falta carga.',
  'train.rec.reason.belowBand': '{reps} reps la sesión pasada; mantén en {lo}-{hi}, sube carga a las {at}.',
  'train.rec.reason.belowBandCluster': 'C{group} dio {reps} reps la sesión pasada; ambos clústeres necesitan {at} para subir carga.',
  'train.rec.reason.underBand': '{reps} reps la sesión pasada — por debajo del rango {lo}-{hi}. Vuelve a {goal} con {load}.',
  'train.rec.reason.underBandCluster': 'C{group} dio {reps} reps — por debajo del rango {lo}-{hi}. Lleva C{group} de vuelta a {goal} con {load}.',
  'train.rec.action.noCall': 'Sin recomendación de carga',
  'train.kind.continuation': 'Continuación',
  'train.kindDesc.continuation': 'Reps pautadas tras un descanso corto — rest-pause o series cluster.',
  'train.addBlock': '+ Continuación',
  'train.structure.auto': 'Auto',
  'train.structureLabel': 'Estructura de series',
  'train.structureUnread': 'aún no se lee',
  'train.structure.straight': 'series rectas',
  'train.structure.myoreps': 'myo-reps',
  'train.structure.restPause': 'rest-pause',
  'train.structure.cluster': 'series cluster',
  'train.structure.drop': 'drop sets',
  'train.structure.superset': 'superseries',
  'train.structure.hit': 'una serie al fallo',

  // ── Rediseño de Entrenar (2026-09-16) ──
  'train.nextUp': 'Siguiente',
  'train.nextNever': 'nunca hecha',
  'train.nextToday': 'hecha hoy',
  'train.nextYesterday': 'última vez ayer',
  'train.nextLast': 'última vez {day}',
  'train.nextDaysAgo': 'última vez hace {n} días',
  'train.nextEditA11y': 'Editar {name}',
  'train.startNamed': 'Empezar {name}',
  'train.startEmpty': 'Entrenamiento vacío',
  'train.tplLast': 'última {day}',
  'train.library': 'Biblioteca de ejercicios',
  'train.libraryTitle': 'Ejercicios',
  'train.librarySearchPh': 'Buscar ejercicios',
  'train.libraryCount': '{n} guardados',
  'train.searchMine': 'Tus ejercicios',
  'train.searchLibrary': 'De la biblioteca',
  'train.searchNone': 'Sin resultados — añádelo con el botón de arriba.',
  'train.prevShort': 'ANT',
  'train.prevA11y': 'La última vez',
  'train.prevNone': 'sin registro',
  'train.setSheetTitle': 'Serie {n}',
  'train.rirExplain': 'Repeticiones en reserva — cuántas más podías hacer. 0 significa que no podías hacer ni una más.',
  'train.exMenuA11y': 'Más opciones para {name}',
  'train.addClusterDesc': 'Serie de activación y luego dos mini-series tras un descanso corto.',
  'train.addBlockDesc': 'Una continuación prescrita tras un descanso corto.',
  'train.deleteSessionTitle': '¿Borrar este entrenamiento?',
  'train.deleteSessionBody': 'Se eliminará el entrenamiento registrado el {name}. No se puede deshacer.',
  'train.audit.summary': '{n} clústeres · {inRange} de {total} en rango',
  'train.structureDesc.auto': 'Sin declarar — la app lee las series y lo deduce.',
  'train.structureDesc.straight': 'Series normales con descanso completo. Progresas subiendo peso.',
  'train.structureDesc.myoreps': 'Una serie cerca del fallo y luego mini-series tras 5-10 s de descanso.',
  'train.structureDesc.restPause': 'Una serie al fallo y luego repeticiones prescritas tras un descanso corto.',
  'train.structureDesc.cluster': 'Una serie y luego continuaciones prescritas tras descansos cortos.',
  'train.structureDesc.hit': 'Una sola serie llevada al fallo.',
  'train.structureDesc.drop': 'Bajas el peso y sigues. La app todavía no las evalúa.',
  'train.structureDesc.superset': 'Dos ejercicios seguidos. La app todavía no los evalúa.',
  'train.structureMore': 'Más estructuras',
  'train.structureFewer': 'Menos estructuras',
  'train.structureKept': 'Mantuvimos tus series — ya tienen números.',
  'train.templateOptions': 'Opciones de la plantilla — notas, descanso, cardio',
  'train.musclesLabel': 'Grupos musculares',
  'train.musclesHint': 'Los usa el conteo semanal de clústeres. Un ejercicio sin ninguno sale como no atribuido.',
  'train.rec.reason.straightSets.hit': 'Aguantaste {target} reps en cada serie por {n} sesiones.',
  'train.rec.reason.straightSets.building': 'La serie más baja fue {reps} de {target} reps — {n} de {needed} sesiones en meta.',
  'train.rec.reason.straightSets.noTarget': 'Este ejercicio no tiene meta de reps.',
  ...
  'train.rec.reason.unsupportedStructure': 'Programado como {structure} — el motor todavía no lee esa estructura.',
  'train.rec.calibrating': 'Calibrando — {n} de {needed} sesiones válidas anotadas.',
  'train.rec.warn.failureOnRir1': 'Anotada al fallo. Este ejercicio está configurado para dejar 1 rep en reserva.',
  'train.rec.settings': 'Ajustes del ejercicio',
  'train.rec.reason.jumpTooBig': 'La siguiente carga disponible es {next} (+{pct}%). Llega a {reps} reps con {load} primero.',
  'train.rec.clusterPrefix': 'C{group}: ',
  'train.rec.invalid.firstMiniTooMany': 'La primera mini fue de {n} reps; más de 5 significa que la activación no estuvo tan cerca del fallo.',
  'train.rec.invalid.firstMiniTooFew': 'La primera mini fue de {n} reps; menos de 2 significa que la activación llegó demasiado cerca del fallo.',
  'train.rec.invalid.miniExceeds': 'Una mini ({n}) superó las reps de la activación ({reps}).',
  'train.rec.invalid.loadChanged': 'La carga cambió entre clústeres — una carga por ejercicio.',
  'train.rec.invalid.repsMissing': 'La activación no tiene reps anotadas.',
  'train.rec.invalid.rirMissing': 'Sin RIR en la activación — anótalo en la serie de activación.',
  'train.rec.invalid.rirTooEasy': 'Activación a RIR {rir} — demasiado lejos del fallo.',
  'train.rec.invalid.minisMissing': 'No hay minis anotadas después de la activación.',
  'train.rec.invalid.notClustered': 'Anotado como series rectas, no como clúster.',
  ...
  'train.rec.stall': 'ESTANCADO — {n} sesiones con {load}.',
  'train.rec.stallInvalid': '{n} de {total} lecturas fueron inválidas — esa es la causa.',
  'train.rec.stallEasy': 'Primera mini por encima de 5 en {n} de {total} — la activación fue demasiado fácil.',
  'train.rec.stallBlocked': 'C{group} bloqueó cada vez.',
  'train.rec.fix.shortenMiniRest': '{n}. Acorta el descanso entre minis a 5-10 s.',
  'train.rec.fix.reduceBlockingCluster': '{n}. Baja C{group} un incremento.',
  'train.rec.fix.reduceLoad': '{n}. Baja la carga 10% y reconstruye.',
  'train.lift.title': 'Ajustes del ejercicio',
  'train.lift.effort': 'Estándar de esfuerzo — serie de activación',
  'train.lift.effortFailure': 'Al fallo (RIR 0)',
  'train.lift.effortRir1': '1 rep en reserva',
  'train.lift.effortHint': 'El fallo es el estándar. Deja 1 en reserva solo donde el fallo bajo carga choca con una restricción.',
  'train.lift.band': 'Rango de reps',
  'train.lift.bandDerived': 'Automático — de tus últimas {n} sesiones válidas con {load}: mantén en {lo}-{hi}, sube carga a las {at}.',
  'train.lift.bandCalibrating': 'Automático — calibrando, {n} de {needed} sesiones válidas con esta carga.',
  'train.lift.bandOverride': 'Manual — sube carga a las {at} reps (mantén en {lo}-{hi}). Automático apagado.',
  'train.lift.bandField': 'Subir carga a las',
  'train.lift.bandReps': 'reps',
  'train.lift.bandHint': 'Déjalo vacío para automático. El rango de mantener son las dos reps bajo la marca.',
  'train.lift.bandClear': 'Volver a automático',
  'train.lift.save': 'Guardar',
  'train.audit.title': 'Clústeres esta semana',
  'train.audit.hint': 'Un clúster = una serie rest-pause. Rango: 2-6 por músculo por semana.',
  'train.audit.below': 'por debajo del rango',
  'train.audit.inRange': 'en rango',
  'train.audit.above': 'por encima del rango — revisa la recuperación',
  'train.audit.unattributed': 'Sin grupo muscular: {names}',
  'train.muscle.chest': 'Pecho',
  'train.muscle.back': 'Espalda',
  'train.muscle.shoulders': 'Hombros',
  'train.muscle.biceps': 'Bíceps',
  'train.muscle.triceps': 'Tríceps',
  'train.muscle.quads': 'Cuádriceps',
  'train.muscle.hamstrings': 'Isquios',
  'train.muscle.glutes': 'Glúteos',
  'train.muscle.calves': 'Pantorrillas',
  'train.muscle.core': 'Core',
  'train.muscle.forearms': 'Antebrazos',
  ...
  'train.kind.mobility': 'Movilidad',
  'train.mobilityDoseNote': "Los estiramientos sostenidos de más de unos 60 s antes de levantar reducen de forma medible la fuerza (−5.4%) y la potencia (−1.9%). Bajo 45 s el efecto es mínimo, y el rango de movimiento sigue mejorando a los 60 s.",
  ...
  'train.kindDesc.mobility': 'Posición o movimiento cronometrado. Nunca cuenta para récords ni progresión.',
  'train.progressionRule': 'Logra {reps} reps por {sessions} entrenamiento(s) seguido(s) → Ignia sugiere añadir {lb}.',
  ...
  'train.glossary.structure': 'Estructura de series',
  'train.glossary.structureBody': 'Cómo está PROGRAMADO un ejercicio — series normales, myo-reps, rest-pause, cluster o una serie al fallo. Lo declaras en la plantilla y decide dos cosas: con qué filas empieza el ejercicio y qué regla usa la app para juzgar si toca subir peso. Déjalo en Auto y la app lee tu lista de series.',
  ...
  'train.glossary.progressionBody': 'Logra tus reps objetivo por cierto número de entrenamientos seguidos e Ignia te sugiere subir el peso. Ajustas los tres números por ejercicio al editar una plantilla.',
  ...
  'settings.redoSetup': 'Repetir la configuración',
  'settings.redoSetupSub': 'Vuelve a preguntarte toda la configuración',
  ...
  'reminder.lapsed3Title': 'Tu plan sigue aquí',
  'reminder.lapsed3Body': 'Registra una comida cuando quieras — con eso basta para retomar.',
  'reminder.lapsed7Title': 'Cuando quieras',
  'reminder.lapsed7Body': 'Una semana fuera no cambia nada. Registra algo para retomar donde lo dejaste.',
  ...
  'settings.exportSub': 'Un CSV con cada comida, peso, agua, sueño, medida y entrenamiento.',
  ...
  'targets.errNumber': 'Escribe un número.',
  'targets.errProteinMin': 'La meta de proteína debe ser al menos {n} g.',
  'targets.errProteinMax': 'La meta de proteína no puede pasar de {n} g.',
