...
**Updated:** 2026-08-29 · **Owns:** current state only. Not history
...
| **OTA (EAS Update)** | Live. `runtimeVersion: {"policy":"fingerprint"}`, channels match build profiles. Free tier 1,000 MAU. **iOS OPEN; Android's channel is SHUT to real users** (§2). Latest: **iOS OTA 54 on build 60 (`7b347b0f…`) — reaches every iOS user, build 60 being the public App Store binary. Android OTA 88–89 went out on `d7ea3629…` = vc 40, which is on NO track, so they reach the test device and NOBODY else; every real Android user is on vc 37 and still has OTA 87.** Publishing to vc 40 is deliberate — it is the only way to device-verify while the channel is shut, and it costs no user exposure. **"Published on both platforms" is not "delivered on both platforms" here.** The iOS crash re-ship is still UNDER TEST (§2) and everything since rides on top of it. **This row is a POINTER, not a log: `apps/mobile/AGENTS.md` is the per-publish record and it wins.** It is deliberately kept to two facts (newest number per platform, and who receives it) — the version that listed a dozen historical publishes went stale twice, once reading "Android 7 / iOS 5" against `AGENTS.md`'s Android 27 / iOS 12. Re-check with `npx eas update:list --branch production --limit 3`, which prints the runtime each went out under |
...
**Milestones (#108/#109) is merged to `feat/milestones` and PUBLISHED, but delivered on iOS only.** PR #116. iOS OTA 54 reaches every iOS user; the Android half sits on vc 40 and reaches only the test device, so real Android users get it when vc 40 reaches a track. Device-verified on the LG VS988 in both themes with the write read back out of PROD Firestore. `firestore:rules` deployed. Detail: the OTA 88–89 row in `apps/mobile/AGENTS.md`. **#110 (`goal-reached`) stays open** — `dailyWeights` carries no `source`, so the app cannot tell a typed weigh-in from an auto-import, and the guard requires manual provenance.
...
after a native crash on reload forced a rollback earlier the same day.

**The crash is unexplained and the re-ship is an experiment, not a fix.** It was
one event on one device with no telemetry — nothing in Sentry, 0
`diagnosticSignatures` on build 60 when asked. `textAlignVertical` is ruled OUT.
OTA 82's diff cannot crash natively, so the frame is `motion.tsx`, the reload
path, or something unrelated. **What the re-ship buys is a decision tree** —
crash on Restart then fine = the reload path, not the ring; crash every time
Today opens = `motion.tsx`; no crash = the first event was transient. Do not
record any of those as the answer until one is observed. Rollback is one
command: republish group `2318d259`. Full write-up: `apps/mobile/AGENTS.md`.
...
| Work | Blocked on |
|---|---|
| **Play DSA trader declaration — the last thing standing between Android and 30 more territories** | **Owner, in Play Console.** Production access was granted and **vc 37 shipped to production on 2026-08-29** (§1) — that work is done and its lessons are in `scripts/play-production-release.mjs`. What remains is Google's own DSA trader declaration, which **Apple's does not carry over**. Until it is filed, the production track stays at 128 and the EU 27 + GB/IS/NO stay off. **Order matters and is not reversible cheaply:** file Play's declaration FIRST, then move the 30 rows out of `EU_PENDING_PLAY_DSA` and re-run `--availability`. Opening them first ships into the EU with no Google-side trader info, which removes the app from all 27. Note the 17 in `NOT_OFFERED_BY_PLAY` can never be opened — Play has no storefront for them, so Android's ceiling is 175 - 17 - (30 if unfiled). |
