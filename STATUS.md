...
| **Trends consolidation is LIVE; "My View" is reopened as #103 and undecided** | **Owner, one question.** Trends went six elements -> **four** on 2026-08-26: a **Weekly** panel (This week ⇄ Budget) and a **Habits** panel (Sleep ⇄ Fasting), with the tab choice remembered per device in AsyncStorage. Live on both platforms (Android OTA 71–72, iOS OTA 39) and device-verified on the LG G6 in both themes, including the choice surviving a cold start. **This is ADR-0034's own ordering — consolidation before configuration — and it moved the reopening trigger FURTHER away, not closer**: the trigger was >8 elements or three users asking, and the count is now 4 with 0 users. **#103 exists because the owner reopened it anyway**, which is a decision and is recorded as one rather than as the trigger having fired. **The question #103 asks and nobody has answered: with four elements, what would actually get hidden?** If the answer is the Habits panel for someone with no wearable who does not fast, the three-state card contract already covers that for free and the sleep stub row is the only thing a user genuinely cannot dismiss. **The seam not to break:** the tab cache is AsyncStorage and must stay that way — option C stores hidden ids on the PROFILE, needing a `firestore.rules` deploy that is cross-frontend, and getting it wrong rejects the frozen web's profile writes too. |
