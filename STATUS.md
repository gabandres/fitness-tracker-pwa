...
**The web SEO content prerender is merged and NOT deployed (2026-08-23).**
`scripts/prerender-seo.mjs` now writes each content route's real copy into the
served HTML. It is verified against a local `dist/` — 113 pages, HTML balance
checked, `ngsw.json`'s hash table untouched (prerendered files are not in it,
so the deploy guard is unaffected) — but **nothing has been deployed**, so
`ignia.fit` still serves the old head-only pages. It reaches users on the next
`npm run build && firebase deploy --only hosting`, which must be a PROD build.
...
| **`bermudezsystems.com` is a single point of failure for the Play org account — OWNER CONSOLE STEPS** | **Owner, and step 1 is blocked on a Northwest login code.** The trigger was small: the first real in-app feedback was `delivered` to `gabriel@bermudezsystems.com` on 2026-08-22 and the owner never saw it — the message was sitting in the Northwest webmail, and the forward to Gmail that this repo recorded as *"owner to configure"* had never been configured. Delivery was fine; the forward is what does not exist. **The forward is not the real risk.** The org Play developer account's Google login IS `gabriel@bermudezsystems.com`, auto-renew on that domain is **OFF**, and the free Northwest year lapses **~July 2027**. If it lapses: the developer-account login goes with it, Apple's org-website precondition breaks, and the GSC TXT record Google's website verification rides on disappears — and the domain becomes registerable by anyone, who then receives mail addressed to the owner, which is a live account-recovery path into his own accounts. Do in this order: **① renew the domain for multiple years, or transfer it to a registrar with a card on file** (load-bearing, and not really about email). **Attempted 2026-08-23 and Northwest's UI will not allow it.** Signed in fine (the Cloudflare/code trap did not fire); the domain row is at Services → Domains → select → **Actions**, where **Auto Renewal** *and* **Renew** are both hard-disabled (`p-select-option p-disabled`) with no tooltip and no reason given. It is **not** a missing payment method — Account → Payment Methods holds **VISA •••• 5311, exp 12/2030**; wallet balance is $0.00 and no autopay option is selected. Both controls being disabled together points at the domain sitting inside the **free first-year Business Identity bundle**, ineligible for renewal until closer to the 2027-07-05 expiry. There is no auto-renew switch anywhere else: the domain's own General tab has none, and Account → Registrations & Renewals is **state filings**, not domains. So the options are (a) ask Northwest support to flip it, (b) transfer to a registrar with a card on file — note the site's `A 199.36.158.100` is Northwest hosting and Apple's org-website precondition rides on it, so DNS must be carried over, or (c) diarise a manual renewal for ~May 2027. **(a) is in flight: emailed `support@northwestregisteredagent.com` 2026-08-23 16:44 AST** from `gabriel@bermudezsystems.com`, subject *Enable auto-renew on bermudezsystems.com*, asking them to flip it or say what makes the option available, and offering to prepay multiple years. The body names `gabriel@bermudezpr.com` as the contact on file, since that — not the LLC address — is what Northwest has on the account and writes to. If they go quiet, Alex Sullivan's 2026-08-06 outreach gives a phone route: **509-768-2249**; **② put non-domain recovery on the org Google account** — recovery email + phone pointing at the personal Gmail and number, plus saved 2FA backup codes, so a lapse cannot lock him out of the developer account; **③ move the mailbox off the free suite** — `bermudezpr.com` is already on a paid Microsoft 365 tenant, so adding this domain there gives real IMAP and no forwarding rule to fail silently. **Priced 2026-08-22, and the answer is $0** — a tenant holds many domains, so adding `bermudezsystems.com` to the `bermudezpr.com` tenant costs nothing, and the mailbox is unlicensed in either shape: an **alias** on `gabriel@bermudezpr.com` (up to 400, same inbox, send-as) or — better here, to keep LLC mail separate — a **shared mailbox**, also unlicensed up to 50 GB and opening as its own folder in the same Outlook. A genuinely separate *licensed* mailbox is the only paid shape and is not needed: Business Basic **$7/user/mo** ($84/yr), Standard **$14**, both after Microsoft's 2026-07-01 increase. The work is DNS at Northwest (MX + autodiscover + SPF/DKIM → Microsoft); it moves **mail only**, leaves the A records and the `google-site-verification` TXT untouched, so neither the site nor Google's website verification is at risk. Cloudflare Email Routing is the other free option but is *forwarding* — the exact mechanism that just failed silently — and it demands Cloudflare nameservers, putting the whole zone at risk to save $0 against $0. The Northwest suite also caps sends at **5/day**, which is not a thing to discover mid-thread with Apple. **Step-by-step runbook, with the live DNS measured: `docs/DEV_ENVIRONMENT.md` §4** — note every TTL that changes is **60s** (cutover and rollback are ~1 min) and that the domain publishes **DMARC `p=quarantine`**, so MX and SPF must move in the same sitting or outbound silently lands in spam. If ③ is declined, the minimum is ① + ② **plus fixing the forward and sending a test through it** — the failure mode was silent, and "it is configured" is not evidence. Meanwhile the feedback ping no longer depends on any of this: it goes straight to the personal Gmail, one constant in `functions/src/feedback-notify.ts` with a `MACROLOG_FEEDBACK_TO` override |
...
| **"Not intuitive for women" — the WHOLE F-series is built, shipped and verified on BOTH platforms. Closed.** | **F0** the tour, **F1/F2** the sex-blind calorie seed, **F3** kilograms (body weight AND barbell loads), **F4** *Search foods*, **F5** the + button clearing *Repeat yesterday*, **F6** a "?" glossary on Today and Trends, **F7** the unsigned `CutPace` that handed a "gain" user a deficit. Details in `UX_AUDIT.md`; user-facing summary in `CHANGELOG.md`. **F3 is the one worth knowing about**: `body-weight-units.ts` and `load-units.ts` are separate modules because per unit the *objects* differ — the bar is 45 lb or **20 kg** (44.09 lb), the plates are 45/35/25/10/5/2.5 against 25/20/15/10/5/2.5/**1.25**, the step is 5 or 2.5. So **plate math and the warm-up ladder are solved in the display unit**; a pound solve converted afterwards tells a metric lifter to load a 20.4 kg plate. **Storage stays in pounds everywhere.** **Device-verified on Android** (80.5 kg → 177.47 lb read back out of PROD Firestore; the 100 kg plate solve reading `25×1 15×1 / side` off a 20 kg bar) and **on the iOS simulator: 17 of 19 Maestro flows pass** on a Release build from `main`, including the two new flows. **The two iOS failures — `15-search` and `18-train-template` — are PRE-EXISTING and that was proven, not assumed**: `27828e5c` was checked out, rebuilt and run, and both fail there too. `.maestro/regression/coverage.md` carries what each one actually shows, from the run artifacts and from Firestore. They are the only open thread here and neither blocks a release |
...
| **#46 — watch layouts at 40mm/46mm, both locales** | **`ignia-mac` DISK, not Xcode.** Re-measured 2026-08-23: the watchOS 26.5 simulator runtime IS installed and the Mac is reachable, so the precondition this row used to name is met — but `/System/Volumes/Data` has **7.5 GiB free** against this repo's own measured ~17 GB floor for an iOS build, and there is no watch simulator *device* created yet either. A prebuild + Xcode archive from that starting point risks filling a machine this project does not own while nobody is at it, so it was not attempted. Free disk on the Mac first; then it is the readout it was designed to be. Still the **last open item** of the 16-ticket Apple glanceable-surfaces map |
