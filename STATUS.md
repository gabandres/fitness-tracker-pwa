...
| **The splash hang is FIXED and live on BOTH platforms — nothing open** | **Nothing.** Android OTA 60 on vc 37 and iOS OTA 32 on build 60, the public App Store. **The measurement is the part worth keeping**: `onAuthStateChanged` fires at **771 ms** on a working network and **8,690 ms** against dead DNS, because Firebase validates the restored token before it will emit — so the fix reads Firebase's own persisted session off AsyncStorage after a 1200 ms grace, which means the presumed path only ever runs when the network is genuinely failing. Black-holed, the app is interactive at **~6 s** against **>30 s** before the first splash fix; the real session lands by 12 s and corrects the avatar with no flap-back. **A first cut shipped and did NOT work**, caught only on the device — the splash lifted off `initializing` and was instantly re-raised because the profile had never started loading. **The reusable tool:** `settings put global private_dns_mode hostname` + a bogus specifier black-holes DNS reversibly while wifi stays `CONNECTED`, which is the only way to test this class here — the LG G6's own wifi fails at random. **iOS behaviour remains unverified** (no iOS device on this project). |
| **ADR-0030 is COMPLETE and ACCEPTED — nothing left; kept only because the iOS half is unpublished** | **One command on a machine that is offline.** Step 3 is finished and `npm run check:day-boundary` is at **BASELINE 0**, failing in both directions. Q5's rule: **we derive the day only when the source did not** — raw samples converted, OS-bucketed daily totals keep the source's day (the OS bucketed a full 00:00–24:00 at calendar midnight, so re-keying its start under 03:00 would file a whole day's steps as the previous user-day), sleep keeps ADR-0033's wake-day rule. The two cardio merge keys moved **together** because they are compared to each other. Q4 needed no code: the widget's `calendarDateKey` is the fallback for snapshots predating `dayEndsMs`, which `widgetView` prefers. **Live on BOTH platforms** — Android OTA 61 on vc 37 and iOS OTA 32 on build 60, the public App Store. **Inert for every existing account** (all on MIDNIGHT). |
