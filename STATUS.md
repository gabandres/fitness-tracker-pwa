...
**Milestones (#108/#109) — PR #116, published, delivered on iOS ONLY.** iOS OTA 54 reaches every iOS user; the Android half sits on vc 40 and reaches only the test device. Device-verified both themes, write read back from PROD, rules deployed. **#110 stays open**: `dailyWeights` has no `source`, so the app cannot tell a typed weigh-in from an auto-import. Detail: the OTA 88–89 row in `apps/mobile/AGENTS.md`.
