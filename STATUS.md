...
| Work | Blocked on |
|---|---|
| **Open the EU 30 on Play** — the sequencing gate cleared 2026-09-03 evening | **Console work, ~30 checkboxes; nothing to file.** There is no Play trader form for a verified org (measured 08-30: DSA requirements folded into org verification, done 08-20). vc 44 is now live, so the countries change no longer resets a release review — it joins the Data safety amendment already in review. Steps: Play Console → Production → Countries/regions → add the 30 in `EU_PENDING_PLAY_DSA` (`scripts/play-production-release.mjs`) → send for review; then move those 30 rows into `ASC_ALPHA3_TO_PLAY_ALPHA2`, delete the set, and confirm with `--availability` → **158** (the 17 `NOT_OFFERED_BY_PLAY` stay unopenable). Read today: 128, restOfWorld=false. |
