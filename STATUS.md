...
|---|---|---|
| **MEASURED 2026-08-15: the junk-mail verification bug was eating ~half of password sign-ups** | **33 accounts created in the last 45 days; 12 (36%) ever wrote a single row; 21 wrote nothing at all.** Narrowing to the accounts the bug could touch: **17 password sign-ups, 8 of which NEVER verified their email — and all 8 logged zero meals.** Google/Apple sign-ups arrive pre-verified and are unaffected. `firestore.rules` blocks every write until `email_verified`, so an unverified account is a walled account, and until 2026-08-14 the verification mail shipped from `noreply@<project>.firebaseapp.com`, which cannot be DMARC-aligned with `ignia.fit`. That is a plausible cause for ~47% of password sign-up loss, and it is the leak that was live during every acquisition push to date. **Do not read `lastSignInTime` as retention** — Firebase only updates it on an actual sign-in event, and sessions persist, so "last sign-in = created" is an artefact, not a churned user. Row counts are the honest signal. **SEQUENCING CONSEQUENCE, and it is the important part: the fix is NOT in the live App Store binary.** Web and Android (vc 30, OTA'd 2026-08-15) have it. iOS gets it when **1.2.0 is approved and released** — and only because the binary under review was swapped from build 54 to **build 55** on 2026-08-15; build 54 did **not** contain it, and build 24 (the live one) has never been an OTA target, so there is no way to reach the public early. **So driving iOS traffic — a flyer, a launch post — before 1.2.0 is live sends new users into the same junk-mail wall this measured.** | `node scratchpad/tester-engagement.mjs` (Firebase Auth × per-user Firestore `count()`) |
...
| **iOS 1.2.0 (build 55) is SUBMITTED to App Store Review — the public has been two versions behind since 2026-08-08** | **SUBMITTED 2026-08-15 with build 54, then RE-SUBMITTED THE SAME DAY WITH BUILD 55**, `WAITING_FOR_REVIEW`, **`releaseType: AFTER_APPROVAL`** — it goes public the moment Apple approves, with no human step. It was set to `MANUAL` for a few hours on the reasoning that the owner might want to time the public moment against a gym flyer; that was changed on the owner's decision, because the cost of `MANUAL` is that an approval landing overnight leaves the verification-email fix unshipped and the flyer blocked until someone notices. `AFTER_APPROVAL` also matches what 1.0 and 1.1.0 did. **Build 54 was the wrong binary and it was caught by reading the artifact, not by reasoning**: it was cut from `ea96f48` and the verification-email fix landed in `86183368`, *after* it, so `main.jsbundle` in build 54 contained **zero** occurrences of `sendVerificationEmail` against one of `sendPasswordReset`. Shipping it would have put the junk-mail path — measured at ~47% of password sign-ups, see the row above — into the very release being rushed to fix it. Build 55 is from current `main`, verifier green, and its bundle carries `sendVerificationEmail` **1** / `sendOwnedVerificationEmail` **1**. Same fingerprint `886bf0b3…` as 50–54, so no OTA is stranded. Swapped with `scripts/asc-swap-review-build.mjs --build 55 --notes store-assets/whats-new-1.2.0.json --commit`, which cancels (forfeiting ~4 h of queue), re-points, sets the notes in the only window ASC allows, and re-submits; new submission `6c54aa0a`. Re-read from ASC afterwards: build 55 attached, both locales 13 lines. Original row follows — approval does **not** publish it; someone presses release. The store has been serving **1.1.0 / build 24** while TestFlight ran to build 53, so every new download carried none of the last week's fixes. **Build 53 could not be the store candidate**: it was uploaded 2026-08-14 09:16 PDT and the TDEE discontinuity fix (`866369ba`) landed 20:36 EDT the same day — 53 has that fix only by OTA, so a fresh install would run the 1,700-kcal maintenance bug on first launch, before any update applies. Build 54 was cut off `ea96f48` on `ignia-mac` at **zero EAS quota**, `IGNIA_BUILD_EXIT=0`, and `verify-mobile-artifact.mjs` passed — four targets, every plist key, both `.lproj`, both URL schemes, `CFBundleVersion 54`. Fingerprint **`886bf0b3d384b8a43477730cdd0124572b954711`**, read from the `.ipa` — **identical to builds 50–53**, which is the JS-only rule holding and means the 08-14 OTA already covers this runtime *and* that 54's embedded JS carries the fix. Disk on the Mac was 10 GB against a 17 GB floor; reclaimed to 11 GB by deleting four already-uploaded `.ipa`s and Xcode `Archives` (the CocoaPods cache was left — deleting it forces a mid-build re-download). What's New written in **en-US + es-MX**; review notes name the five specific changes per the 2.3.1 rule in §6 and declare the new microphone/speech permission; demo account `review@ignia.fit` confirmed verified, enabled and seeded. **The watch complication and Siri quick-add are deliberately NOT advertised** — both are still `BEHAVIOUR UNVERIFIED`, so they appear as "fixes" and are claimed to no reviewer. **Build 54 was NOT distributed to external TestFlight testers**: 53 is already external and functionally identical under OTA, and adding a beta submission while a store review is in flight buys nothing | `node -e` on `/v1/appStoreVersions` → `1.2.0 WAITING_FOR_REVIEW` |
| **Every email Ignia sends was unaligned with `ignia.fit` — FIXED 2026-08-14/15, both halves** | **Half one, Resend mail: FIXED AND DEPLOYED** (`sendWelcomeEmail`, `sendPasswordReset`, `hourlyTasks`). `mail.ignia.fit` has been `status: verified` / `sending: enabled` since **2026-07-24**, DKIM + SPF live in Cloudflare, DMARC `p=none` on the apex — and the sender was never flipped, so for **three weeks** every welcome, password-reset and weekly-digest mail shipped from `onboarding@resend.dev`, Resend's shared sandbox. `docs/email-deliverability.md` read "awaiting Resend's verification poll" the whole time, which looks like *blocked* rather than *stale*. Fixed at the **tracked default** (`FROM_FALLBACK` in `functions/src/resend-client.ts`), not in a `.env` — a fourth place to forget is what caused it. **Reply-To deliberately stays a personal Gmail**: `hello@ignia.fit` has no MX and would black-hole every reply. **Half two — the one that actually junked a user's mail: Firebase Auth's OWN verification mail, now ours too.** It never went through Resend; both clients called `sendEmailVerification`, so it shipped from `noreply@<project>.firebaseapp.com`, which no DNS record on `ignia.fit` can align. **The expected fix — Auth → custom SMTP — IS NOT AVAILABLE ON THIS PROJECT**: every write to `notification.sendEmail` returns `400 EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`, including a single-field one, because `enableImprovedEmailPrivacy` is on. **Do not turn that off to unlock it** — it is why `fetchSignInMethodsForEmail` returns `[]`, and the callable below costs nothing in exchange. Shipped instead: **`sendVerificationEmail`** (`functions/src/verify-email.ts`), mirroring `sendPasswordReset` — `generateEmailVerificationLink()` + Resend + the branded `Block` templates in `en`/`es-PR`. **Authenticated**, so the address comes from the token and enumeration is structurally impossible; rate limited 5/hour per uid; failures surface rather than resolving to a cheerful `ok:true`. `brandActionLink()` (`auth-links.ts`) also moves the link host from `firebaseapp.com` to `ignia.fit`, which serves the byte-identical OOB handler — **applied to password-reset links too**. LIVE on web (prod build + hosting deploy, `sendVerificationEmail` read back out of the served `main-5VESEJA2.js`) and deployed as a function; the unauthenticated call returns `UNAUTHENTICATED`, verified. **The mobile half is written and merged but is in NO binary and has NOT been OTA'd** — deliberately held while 1.2.0 sits in App Review; publish it after approval | `curl -s https://ignia.fit/main-5VESEJA2.js \| grep -c sendVerificationEmail` → 1 · Gmail *Show original* → `dkim=pass header.d=mail.ignia.fit` |
| **TDEE: one logged meal could move maintenance by 1,700 kcal — FIXED, live on web + OTA'd to both mobile runtimes** | **SHIPPED 2026-08-14** from `866369ba`. Web: prod build + `firebase deploy --only hosting`, and the served chunk was fetched back and compared **byte-for-byte** against the local build (`chunk-SU6QGVWL.js`, identical, clamp constant present) rather than inferred from the deploy exit. Mobile: gate run on `ignia-mac` first and **both fingerprints matched the live binaries exactly** — ios `886bf0b3…` (builds 50–53), android `65199166…` (vc 30); groups `64050b9d` ios / `be08b390` android. **The defect was a discontinuity, not a bad estimate.** A real account stopped weighing 07-11 → 08-04; `lastTrendSegment` cut there and `POST_BREAK_SETTLE_DAYS` left a run starting 08-11. At **three** points that run was below `MIN_SEGMENT_POINTS` so the whole 25-point window carried the fit — −0.0044 lb/day, maintenance **1,889**. Logging one more day pulled in a fourth weigh-in, the count was met *exactly*, and the fit switched to a line through four **consecutive daily** readings (159.6 / 158.2 / 158 / 158) — a three-day span dominated by one 1.4 lb overnight drop, read as **−0.50 lb/day**, an implied deficit of **1,750 kcal/day**, maintenance **3,596**, target **3,146** against an 1,850 floor. The floor only clamps from below, so nothing caught it. **Counting points cannot see this** — four daily weigh-ins clear the count and still span three days. Two changes: a post-break run must now **span a full calendar week** (`MIN_SEGMENT_SPAN_DAYS = 6`, because `x` is a day offset and seven distinct days span 6), and `MAX_TREND_LBS_PER_DAY` clamps the slope at 2 lb/week through a single `finish()` so it cannot rot on one of the four fallback paths. **14 was tried first and broke the 2026-08-11 design** — the step/rebound scenarios leave exactly one week after the settle window, so a 14-day floor reverted both to the whole-window fit they exist to replace (2,500 → 2,392). Those tests are why the number is 6. **Verified on the real account: logging today now moves maintenance 1,889 → 1,864.** Three regression tests built from its actual weigh-ins. Suites: core 778 · mobile 203 · web 204. What's New bumped to `2026-08-14` in both locales | `curl -s https://ignia.fit/chunk-SU6QGVWL.js \| grep -c 2857142857` → 1 · `npx eas update:list --branch production --limit 2` |
| **iOS build 53 — the watch push CANNOT wake a WidgetKit complication, and that ends the real-time goal** | **`VALID` AND LIVE TO ALL TESTERS 2026-08-14** — `internal: IN_BETA_TESTING`, `external: IN_BETA_TESTING`, in **both** *Team (Expo)* and *Public Beta Testers*, `autoNotifyEnabled`. Beta App Review **auto-approved** (1.2.0 had already cleared it with build 49), so no wait. **This is the build every tester should be on; it supersedes 49, which is what they had until now, and carries 50–52 in full.** Verifier green, 1.2.0, same fingerprint (`886bf0b3…`). **Read this before touching the watch again.** `transferCurrentComplicationUserInfo` wakes a sleeping watch app **under ClockKit**. This app's complication is **WidgetKit** (`StaticConfiguration`, `.accessoryCircular/Rectangular/Inline`) and never was ClockKit. For a WidgetKit complication the transfer does **not** wake the app — `didReceiveUserInfo` fires only once the app is already running. Apple **FB12926788**, filed 2023-08, acknowledged, still open, duplicate reports, public sample project. That accounts for every observation with nothing left over: the phone recorded `sent` with the complication on the active face and 32 transfers left (so the phone was already correct), the watch app showed the right numbers the moment it was opened, the face never moved on its own, and **build 51's reload dedupe changed nothing because the budget was never the cause**. **Real-time delivery to the wrist is not achievable on this surface** — that is a requirement change, not an open bug. What ships instead is a **pull**: `WKApplicationDelegate` + `scheduleBackgroundRefresh` wakes the watch app on the only cadence watchOS offers (~hourly for an app in the dock, 4s CPU) and reads `receivedApplicationContext`, which holds the phone's latest value whether or not any wake happened. Staleness goes from "until you open the watch app" to about an hour. The SwiftUI `.backgroundTask(.watchConnectivity)` modifier was removed with it — `handle(_:)` now receives every background task, and two owners for one task means `setTaskCompletedWithSnapshot` can be called twice, which is a crash on a background wake. **Also**: `LogQuickAddSlotIntent.perform()` was `@MainActor`, copied from the spoken intents that need it for `requestValue`; this one draws nothing, and `LiveActivityIntent` makes the system launch the **app**, so it queued behind React Native saturating the main thread at startup. It now records `perform()` duration + warm/cold to the App Group, surfaced in Settings → Apple Watch. **ADR-0023 is amended, and decision 4 (`LiveActivityIntent`) is back under review**: it costs an RN launch per chip tap and its payoff was real-time watch delivery, which does not exist. **BEHAVIOUR UNVERIFIED** | ASC command below · Settings → Apple Watch |
| **iOS builds 51 + 52 — the watch's own reload budget, and the widget's latency** | **51's premise was wrong — see build 53 above.** The dedupe it ships is still a real fix (the watch was spending a metered 40–75/day reload budget on identical bytes, where the phone has deduped since 2026-08-10), it simply was not the cause of the stale face. **BOTH `VALID` ON TESTFLIGHT 2026-08-14** (read from the ASC API), verifier green, 1.2.0. **Install 52 — it carries 50 and 51 in full.** Internal group only, like 50; external distribution is still the owner's call. **Same fingerprint as build 50** (`886bf0b3…`) — both are Swift under `targets/` only, the documented rule holding. **51 is a real defect, found by reading the watch code after the phone's diagnostics cleared the phone.** `WatchReceiver.store()` wrote and called `reloadAllTimelines()` **unconditionally** — `start()` runs it on every app launch with whatever `receivedApplicationContext` holds (usually last time's bytes) and `ingest()` runs it **twice per background wake**. That call is metered at 40–75/day; once spent, WidgetKit throttles and the face stops moving, which from the wrist is indistinguishable from a delivery that never arrived. **The phone has deduped against exactly this since 2026-08-10 and the watch never did** — the transport's cheap end was careful and its expensive end was not. 51 also adds the watch-side instrumentation the device has never had: which callback stored (`userInfo` can only come from `transferCurrentComplicationUserInfo`, so seeing it proves the wake happened) and when the complication last built a timeline, both shown on the watch mirror screen. **52 is the widget latency the owner reported.** `applyOptimistically` ran **after** both network round trips despite its name and its doc comment, and since build 50 also behind `assertToWatch`'s up-to-3s activation wait — a regression build 50 introduced. The write path is now `stage` (synchronous, local: bump snapshot, request reload, park the row) → `commit` (network, unpark, watch push last). The widget intent uses `logDeferred` and returns in milliseconds, which is the mechanism that matters: a `reloadTimelines` requested from inside `perform()` is **deferred** and documented as sometimes landing minutes later (FB11522170), while the reload the system performs **when `perform()` returns** is the reliable one. The park moved **before** the attempt — a write-ahead log — so returning early cannot lose a row; `flushPendingLogs` replays it under the same id, which is an idempotent overwrite and exactly what `newLedgerId` was built for. **BEHAVIOUR UNVERIFIED on both** | ASC command below · watch mirror screen, bottom line |
| **iOS build 50 — the watch push stops being eventual (ADR-0023)** | **`VALID` ON TESTFLIGHT 2026-08-14** (read from the ASC API), built locally on `ignia-mac` at **zero EAS quota**, 1.2.0, verifier green on all four targets. **INTERNAL TESTERS ONLY so far** — it is in *Team (Expo)*, not in *Public Beta Testers*, which build 49 is in. That was left deliberately: external distribution needs Beta App Review and would push an unverified change to a shipped button out to everyone. **Owner's call, after the device reading below.** Fingerprint **`886bf0b3d384b8a43477730cdd0124572b954711`**, read from the `.ipa` — **its own runtime**, because `modules/watch-link/ios` changed and that directory IS hashed, so no OTA crosses between it and build 49. **What it closes:** a meal logged with the app *not* in the foreground moved the phone's widget and left the wrist stale. Two causes, not one. (1) `assertToWatch` guarded on `activationState == .activated` and returned silently otherwise — but `WatchLinkModule` activates `WCSession` **asynchronously** at launch, so an intent performed on a cold launch runs inside that window and the guard was answering *no* to a question whose real answer was *not yet*, dropping the complication transfer **and** the application context. It now parks the envelope in the App Group first, waits up to 3s for the owner to finish activating (no wait at all when warm), sends over both queues, then clears the park; `WatchLinkSession` drains that park on `activationDidCompleteWith`, which is milliseconds later and before React Native has booted. It still never calls `activate()` and never takes the delegate. (2) A widget chip tap runs in `Today.appex`, where `WCSession` does not exist at all — so `LogQuickAddSlotIntent` now conforms to **`LiveActivityIntent`**, Apple's documented lever for performing an intent in the app's process, launching the app in the background when needed. **BEHAVIOUR UNVERIFIED — no complication has been watched to move after a meal logged outside the app.** The one thing the artifact does prove is that the conformance survived extraction: `systemProtocols: ['com.apple.link.systemProtocol.SessionStarting']` is present on `LogQuickAddSlotIntent` in **both** the app's and the appex's `Metadata.appintents`, and on neither other intent. **This changes where a shipped button runs**, which is why every push now records `{outcome, atMs, process}` to the App Group and **Settings → Apple Watch shows it raw**: a chip tap reporting `parked:appex` means the routing did not take, and that reading replaces what would otherwise be another speculative build. Read it there before changing any code on this surface | ASC command below · Settings → Apple Watch → *Last watch push* |
| **The widget wide face — VERIFIED ON A DEVICE, and it took four builds** | **iOS build 49 was the one testers should be on until build 50 (above) superseded it**; Android vc 30 + an OTA off `86f49aba`. This is the first native surface this project has actually watched working, and it only worked because the owner sent screenshots. Three defects, none catchable by `verify-mobile-artifact.mjs` (which passed on every build): (1) picking the rectangle **crashed** — a `ForEach` over `QuickAddSlot.id`, which is `presetId`, so two slots on one preset collide, plus pattern matching inside a `ViewBuilder`; fixed in `c693fd22`. (2) The face rendered but read as **empty** — `Metric.progress` is computed for every face and no home face had ever drawn it; bars added in `61b72415`. (3) On **tinted/clear** Home Screens the quick-add chips drew as **solid blobs with no text** — `.accented` rendering flattens all colour to one tint, so a filled capsule and its label became the same colour; stroked instead of filled in `2d78e8af`. That third one **predates the wide face** — the small face has had it since ADR-0020 and nobody had tinted a Home Screen to find out. `index.swift` had a standing note that `widgetRenderingMode` semantics were "unverified"; the screenshots are the verification | `apps/mobile/AGENTS.md` fingerprint table · `npx eas update:list --branch production --limit 2` |
| **The widget has a wide face — N4b** | **SHIPPED 2026-08-13 as iOS build 46 (TestFlight, `VALID`) and Android vc 30 (Play alpha)**, off `ab19dd97`. iOS gains `.systemMedium` with a purpose-built `HomeWideView`; Android's provider XML had **`resizeMode` absent, which defaults to `none`** — that was the entire "square only" limitation, now `horizontal\|vertical`, with `widgetInfo.width` (always available, never read) driving a row layout at ≥220dp. Both artifacts passed `verify-mobile-artifact.mjs`; `app-version.json` synced 29→**30** and deployed, so the update banner fires. **Fixed a latent bug both platforms shared**: the quick-add control hardcoded slot index 0, indistinguishable from correct while one chip was drawn, but on a three-chip row it logs the first preset three times — wrong food, silently, numbers moving as if it worked. 6 tests lock it. **Behaviour UNVERIFIED — no wide widget has been placed on a home screen.** | `node scripts/app-version-sync.mjs --check` · ASC `/v1/builds` shows 46 `VALID` |
...
| iOS App Store | **1.1.0, build 24** is what the public downloads (`READY_FOR_SALE`). **1.2.0 / build 54 is `WAITING_FOR_REVIEW` as of 2026-08-15**, `releaseType: MANUAL` — approval does **not** publish it, someone has to press release. The row here previously said "1.0.0, build 7", which had been wrong since 1.1.0 released | ASC command below |
...
| **App Review** | build **55** (1.2.0), `WAITING_FOR_REVIEW` since 2026-08-15, **`AFTER_APPROVAL` — it self-publishes on approval**. Build 54 was swapped out the same day; it predated the verification-email fix | nothing — this is the submission |
...
- **The Apple Watch complication and its transport** — the watch app
  ...
  **AMENDED 2026-08-14: that confirmation was narrower than it read.** It covers a
  meal logged **in the app's foreground**, which is the only case anyone tested.
  A meal logged from Siri (app cold or backgrounded) or from a widget quick-add
  chip reached the phone's widget and never reached the wrist at all — measured
  on the owner's device against build 49 on 2026-08-13/14. Two causes, both
  fixed in **build 50** (ADR-0023, §1): an activation-state guard that treated
  "not yet" as "cannot", and a chip tap running in a process where `WCSession`
  does not exist. Still unverified on a wrist.
  What remains unproven beyond that is the layouts at 40mm/46mm in both locales (#46, needs a simulator). What is still unproven beyond that: the layouts at 40mm/46mm in both locales (#46, needs a simulator). What *was* verified locally: `tsc` clean, both `expo-target.config.js`
