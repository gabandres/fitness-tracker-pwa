...
| **LLC migration — GCP side DONE 2026-08-30; the Cloudflare domain move is the one piece left** | **Owner, runbook `docs/DEV_ENVIRONMENT.md` §5.1.** Done and verified: org account is project Owner (§5.0, invite accepted); billing account **`01916B-2927E2-E01DC7` "Bermudez Systems LLC"** (LLC payments profile, Visa ••5311 by the owner's call, $300 trial credit to 2026-11-29, upgraded to pay-as-you-go so nothing expires); Ignia moved onto it; billing export re-enabled there (Ignia/`billing`); the old account's export disabled so only ONE `gcp_billing_export_v1_*` table family lands in the dataset `cost-model.ts` reads. Billing Account Admin for the personal Gmail: GRANTED same day (2 admins on the account). Domain: ignia.fit turned out to be on **Cloudflare Registrar** (auto-renew scheduled, exp Jul 2027) in the gabriel@bermudezpr.com account; registrant **fully moved to the LLC and CONFIRMED 2026-08-31** — the Contact table reads Bermudez Systems LLC / gabriel@bermudezsystems.com / +1 307 201 8420 / 30 N Gould St, Sheridan WY, no pending state. **The LLC migration is COMPLETE**: stores, project ownership, billing account + export, and the domain registrant are all the LLC's. All four domain contact rows (Registrant/Admin/Tech/Billing) carry the LLC's org, email, phone and Sheridan address — verified 2026-08-31. Sole remainder, optional: the Cloudflare account LOGIN is still gabriel@bermudezpr.com via GitHub SSO. Delete this row on the next STATUS sweep. |
