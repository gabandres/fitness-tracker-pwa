...
| **Play production** | **NOT launched, but access is GRANTED (2026-08-29).** Tracks read live: `production` **empty**, `beta` empty, `alpha` completed vc 37, `internal` vc 39 — **zero Android production users.** The production track's own country availability reads **0 countries, restOfWorld=false** — available nowhere, which is why two refused release attempts published nothing. §3 owns what is left, and it is a Console page, not a command |
...
| Work | Blocked on |
|---|---|
| **Play production GRANTED 2026-08-29. The release is BLOCKED on one Play Console page, and "it is ONE command" — which this row said until today — was wrong.** | **Owner, in Play Console -> Ignia -> Production -> Countries/regions.** Google granted production access on 08-29. `node scripts/play-production-release.mjs --commit` was then run and **Play refused it twice, which is how the real constraint was found:** (1) `Country targeting is only supported for staged releases` — a `completed` release may not carry `countryTargeting`; (2) `The first release on a track cannot be staged`. Those two together are a closed loop: the first production release **must** be `completed` -> which **must not** carry targeting -> so it inherits the **track's** country availability -> and `edits.countryavailability` is **read-only** (`get` and nothing else). **So the 145 territories must be picked by hand in the Console BEFORE the first release exists. No flag can substitute for it.** The track reads **0 countries, restOfWorld=false** today — available nowhere, which is the safe default, and why nothing shipped by accident. **The loop, and it is self-checking:** set the countries -> `node scripts/play-production-release.mjs --availability` prints the exact missing/extra diff against the 145 -> repeat until clean -> `--complete --commit`, which **REFUSES to publish** unless the track matches the table with `restOfWorld: false`. **Removal lists are in the script header**; the DSA-critical 30 are the EU 27 + GB, IS, NO, and **18 more Play offers that iOS does not** — of which **LI (EEA, so DSA applies) and GI** would be caught by "select all minus the EU 27", and **CU, IR, SD, SO** are US-sanctioned and deliberately absent from Apple's list. **"Select all, remove the EU" is therefore NOT safe** — 17 further territories are in the mirror but were absent from the alpha snapshot; Play's API accepted all 17, `XK` included, so they exist and must be ADDED. Promotion still needs no build: vc 37 keeps runtime `ae526937…`, so production users immediately receive every OTA already published. **The 08-28 sequencing decision stands and is now urgent** — vc 37 establishes the track while nothing is at stake, so vc 40 (Block Store, 30 Sept) is a routine SECOND release. **Two of the six application answers were the owner's own testimony** (testers recruited personally, friends/family, no paid provider; how easy: Difficult) — revisit if Google queries either. |
