...
| **Move `ignia.fit` + GCP billing under the LLC; the org account has NO access to the Firebase project yet** | **Owner, ~30 min total, runbook `docs/DEV_ENVIRONMENT.md` §5.** Step 5.0 first (Owner invite to the org account — the API refuses `SOLO_MUST_INVITE_OWNERS` on a no-org project, so it is a console invitation accepted from Outlook). Then the Cloudflare domain move (export the DNS zone FIRST — it carries the site, the Firebase TXT and the inbound-mail MX) and a NEW LLC billing account for Ignia alone, which is also ~$0.30/mo cheaper (free tiers are per billing account). The BigQuery billing export enabled 2026-08-30 belongs to the OLD account and must be re-enabled on the new one when that happens. |
