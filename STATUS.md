...
| Work | Blocked on |
|---|---|
| **Play production access — APPLIED 2026-08-26, awaiting Google (≤7 days). THE TRAP IS DISARMED.** | **Nothing but the email — and when it lands the release is ONE command:** `node scripts/play-production-release.mjs --commit`. **The trap it closes was verified, not assumed:** `countryTargeting` is `null` on the live alpha release, which means *inherit the track* — all 177 Play countries, EU 27 included, and an app distributed in the EU with no DSA trader declaration is removed from all 27. The script pins the release to exactly the 145 territories iOS ships to, with `includeRestOfWorld: false`. **Its own header carries the full reasoning, the ASC mirror rule (`--check-source`, in sync 145/145) and the `XK`/`XKS` Kosovo exception — read it there rather than duplicating it here.** Promotion needs no build: vc 37 keeps runtime `ae526937…`, so the same binary moves tracks and production users immediately receive every OTA already published. The service account holds *Release to production*. **Two of the six application answers were the owner's own testimony**, answered on inference (testers recruited personally, friends/family, no paid provider; how easy: Difficult) — which is what to revisit if Google queries either. |
| **DSA trader status — the last 30 iOS territories. STEP 2 IS NOW ONE COMMAND; STEP 1 IS STILL THE OWNER.** | **Owner, and ONLY for the declaration itself.** Re-measured from the ASC API 2026-08-27: **145 of 175 available, 30 blocked, and the 30 are the EU 27 — all of them — plus GBR, ISL, NOR.** EU distribution triggers Apple's trader declaration, which **publishes** the trader's name, address, phone and email on every EU product page; values are in `CLAUDE.local.md` — the Sheridan address and `+1 307 201 8420`, never the personal number. **That declaration is deliberately not automated** — `play-production-release.mjs` already states the position in its own header ("a disclosure the owner makes personally, not one to stumble into by ticking a box") and this holds the same line. Set it at *ASC → Business → Trader Status* on the **LLC** entity (`94754902`). **Then step 2 is `node scripts/asc-eu-territories.mjs --commit --trader-status-is-live`** — new 2026-08-27, preview by default, and its `--commit` **refuses** without that flag because **the wrong order is destructive**: flip the territories while the entity still declares "not a trader" and the app is REMOVED from all 27 EU territories, which is worse than today and costs a re-review to leave. No API reports the declaration, so the flag is a person asserting it. `availableInNewTerritories` stays **FALSE** and the script never touches it, so a territory Apple adds later cannot silently defeat the carve-out. **The API shape that cost two failed attempts is now encoded rather than remembered:** `POST /v2/appAvailabilities` is a CREATE and 409s post-launch; the mutation is `PATCH /v1/territoryAvailabilities/{id}`, one at a time, with opaque ids that must be READ, not constructed. |
| **#107 Zero-Tap Sign-In — BUILT on `feat/zero-tap-block-store`, NOT merged. A 30 Sept deadline decides which implementation counts.** | **Play production access and one question for Play support — the Health-apps deadlock is CLEARED (2026-08-27).** R8 shipped and is device-verified in vc 39 (issue closed), bitmaps are fine, dynamic memory needs production traffic. Zero-Tap is all that is left, and the shape changed. Play names the **Restore Credentials API**, whose own guide makes *"set up a relying party server"* a prerequisite — it is WebAuthn end to end, and this project has no app server by design. **Google accepts a Block Store integration instead, "only if completed and in production on or before September 30, 2026"** — no server, no new secret, no new collection in a rules file already at Firestore's 1,000-expression limit. That is a day of work against several, and it is done: an Android-only Expo module storing Firebase's own persistence blob with the access token blanked (<1 KB), written **only** under end-to-end encryption, deleted on sign-out and on account deletion. **ADR-0035.** **Measured, and it is why the dependency lives in `modules/` and not `app.json`:** Android moved `ae526937…` → `5facf778…`, **iOS did not move at all**. Via `app.json` it would have moved both and shut the iOS channel while build 60 is public — the 2026-08-25 failure. **On a branch deliberately**: merging shuts the Android OTA channel until vc 40 is on the track. **The restore leg is UNVERIFIED and cannot be verified here** — it fires only on a real Android-to-Android migration, which needs two devices; 13 tests pin the guards, which is not the same claim. **Play support ticket FILED 2026-08-27** from the ORG account (gabriel@bermudezsystems.com), topic *Android vitals and technical quality* / common issue *Restore credentials*, app-scoped to Ignia; **Case ID pending**, reply by email within 2 business days — read it back at Play Console -> Help -> *Your support tickets*. It quotes the two sentences on Google's own page that contradict each other and asks three questions: does "in production" mean the production track; does a testing track before 30 Sept count if production access lands after it; is a qualifying Block Store integration compliant indefinitely. **Play's AI pre-triage answered all three, pessimistically, and it is NOT authoritative** (its own banner says it may be wrong) — which is why the human ticket was taken anyway: *"in production" refers specifically to having the compliant release active on the production track*; internal or closed testing before 30 Sept does **not** meet it; production access granted after 30 Sept voids the Block Store exception **even if the bundle was on a testing track first**, leaving Restore Credentials as the only route to April 2027; and an integration that *was* in production by 30 Sept stays compliant with no migration. **Plan against that reading until support confirms or contradicts it in writing**: it makes 30 Sept a production-track deadline that Ignia does not control, since production access was only applied for on 26 Aug. If the date is missed the branch still delivers the UX and nothing is wasted. |
| **Photo scan — two things have still never been seen on a device** | **Owner, with a phone.** Multi-photo capture is closed, ADR-0029 is `accepted`, and multi-photo capture was confirmed working by the owner on a real meal (2026-08-26) — the quota fix that confirmation forced is in `CHANGELOG.md`. **The asymmetry it established must survive:** `spendCeiling` counts IMAGES because it is the solvency guard, `dailyQuota` counts SCANS because it is the fairness one. Still never observed: **repeat detection has never fired** (needs a My Foods entry matching a typed note; it is deliberately quiet, so "never suggested anything" and "broken" look identical) and **`measured` has never appeared** (needs a legible scale AND unit in one photo). **The seam not to break:** `measured` drops if `clampGrams` altered the number, so a badge never names a weight the scale did not show. **Known-weak neighbour:** de-duplication is improved, not solved — one run sent the same photo three times and returned two hamburger patties. **Owner-only call:** whether a matched repeat should land on an editable draft rather than logging straight to Today. |
| **Resolver head-noun mis-matches — narrowed to ONE surface, and it is the photo one** | **Nothing; a dedicated sitting.** Three classes are now fixed: substring matches and meat analogues (2026-08-26), and a variety name the corpus cannot portion (`1/2 hass avocado`, 2026-08-27 — `CHANGELOG.md`). **The third fix does NOT reach the remaining cases, and the reason is a surface split worth knowing before costing this:** there are TWO resolvers. `headAnchoredGeneralisations` / `generalisesFood` live in `packages/core` but are wired only into the mobile meal-text path (`apps/mobile/src/lib/mealResolution.ts`); the photo path resolves through `functions/src/photo-resolve.ts` + `usda-db.ts`, which still has only the older `headAnchoredTokens`. **What is left is the photo resolver having no abstain path** — it keeps dropping tokens until a 13k-row index yields something, and something always exists: `unsalted butter` returns *Pretzels, soft, unsalted, buttered*, where both words are genuinely present, so only head-noun awareness catches it. That means retuning a ranker against the real dataset. **Treat it as high-risk:** the first attempt at a much simpler guard returned `null` for every single-token phrase — `bacon` resolved to nothing — and the 44-test suite did not catch it; a bare word tested by hand did. There is now a test pinning it. The remaining cases are listed in the header of `functions/test/photo-resolve-description-hitrate.spec.ts`; re-run with `--reporter=verbose --disable-console-intercept` or the output is swallowed. **Porting the core predicates to the photo path is the obvious first move and is NOT free** — the measured harm table in `generalisesFood`'s doc comment is what a naive relaxation costs. |
...
| **The website is invisible to Google — COSTED 2026-08-27, and the answer is NOT the router migration** | **Owner go/no-go on a cheaper plan.** The content half is deployed and verified live: every content route ships its real copy in the served HTML, both locales, with a build-time guard. **Measured today, that moved nothing** — `node scripts/gsc.mjs inspect` returns *URL is unknown to Google* on **7 of 7** sampled URLs, unchanged from the 2026-07-29 baseline, 29 days after the content shipped. **Server-rendering the body of a URL Google never requests changes nothing**, which is why the router migration is now deferred rather than scheduled. **The binding constraint is discovery, and it is measured:** the built sitemap has **118** URLs, **18** are reachable by a real `<a href>`, so **100 are orphans** — including every calculator variant and all **57 Spanish URLs (0 links to any `/es/` path)**. `routerLink` is in zero files, but the app already ships **20 plain anchors**, so the fix is more anchors and needs no router at all. **The migration was costed, not hand-waved:** 21 route branches, 12 SEO components (only ~14 browser-global refs between them — the cheap half), against a shell `app.ts` with 28 plus 8 services and 7 utils on the server-render path, and one hard blocker — `app.ts:503` is `signal(!navigator.onLine)` in a FIELD INITIALISER, so it throws at construction before any route renders. That is the largest change anyone would make to a frontend that is FROZEN (ADR-0022), spent on a surface whose retirement is itself awaiting `usage-report.mjs`. **Order: get the sitemap fetched (`lastDownloaded` was `never` on 2026-08-17), link the 100 orphans, re-measure — then the router migration has an evidence base or does not.** Full costing and the tables: `docs/seo-status.md`. |
