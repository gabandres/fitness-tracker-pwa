...
| **"Not intuitive for women" — the TUTORIAL she asked for is SHIPPED; the sex-blind calorie seed is not** | **Half done. What remains is the calorie seed (F1/F2), approved but not built.** The report arrived in two pieces: one relayed sentence, then — on being asked — the actual ask, **"she needs some kind of tutorial or walkthrough of the app"**, with the owner's framing that *"for men we typically just start tapping and figuring things out"*. That intuition has research behind it: Burnett et al.'s GenderMag work names five problem-solving facets that cluster by gender, including **learning style (tinkering vs process-oriented)** and information processing style (the strongest reported effect, p = 0.0003). **The research asks for a tutorial anyone can take, NOT a gender-branched UI** — these are statistical clusters, not rules about individuals, and a branched UI would be both condescending and unshippable. Requirements that follow from the facets rather than from taste: a guided path with a stated order and an end; the whole map shown before the first step; explicit "you can change this later" (most of the app IS reversible and nothing says so); skippable AND replayable from Settings. **F0 SHIPPED 2026-08-22** — a six-card guided tour, `apps/mobile/src/app/tour.tsx`, auto-opening once per device and replayable from Settings → How Ignia works, live on Android (OTA numbers 12–15, vc 37) and iOS (OTA number 7, build 60). **Device-verified on Android end to end**; UNVERIFIED on iOS as usual. It took four Android publishes because the device rejected what the suite passed — details in the OTA rows in `apps/mobile/AGENTS.md`. **F1/F2 remain: the owner approved adding sex/height/age to onboarding so the seed uses the app's own Mifflin-St Jeor path instead of weight × a constant. Not built.** F3–F6 wait on real words. All of it is JS-only — no native surface, so nothing here touches the Apple migration precondition. The whole report is one relayed sentence: no author, no platform, no screen, no task. It is **not** in the in-app feedback system — that collection group holds two documents, both already handled — so there is no raw text anywhere. The owner confirmed 2026-08-22 that nothing more is retrievable, so a structured first-run walkthrough was run instead (LG VS988, vc 37 + OTA) and six findings are recorded in `UX_AUDIT.md` under *From a second-hand report*. **The one that is arithmetic rather than taste:** onboarding calls `computeKcal(weightLb, goal)` = weight × {11|14|17} — no sex, height, age or activity — while the app's own Mifflin-St Jeor path uses all four. Measured against it at the app's own 1.375 bucket, with CDC/NCHS **measured** NHANES heights (5'8.9" male, 5'3.5" female) so only sex and height vary, that seed runs **+11% high for a 150 lb woman and −6% low for a 150 lb man**; at 180 lb / 45 y it is **+27% high for a woman**, so her "lose fat" target of 1,980 kcal lands **2 kcal/day ABOVE** her estimated maintenance — **0.00 lb/wk, she loses nothing ever** — against 0.69 for the man, a **2.4×** gap. Verified against primary sources 2026-08-22: the Mifflin-St Jeor constants (+5 male / −161 female), the 1.375 multiplier and the NHANES means all check out; the 3,500 kcal/lb conversion is the app's own constant and is a static rule that *overestimates* real loss, so those rates are the optimistic case. `dailyTargets` keeps that number until a measured estimate turns reliable (weeks), and the formula path cannot rescue it because `toProfileFields` returns null without `sex`/`heightIn`/`age`/`activityLevel` — all four of which are collected **only** in Settings → Refine targets, sold as *"Sharpen your calorie target"*. **Do NOT design from the paraphrase and do not infer a redesign from gender** — the precedent is Abdiel's calorie-goal item, where the summary said "no custom option" and the real defect was a number being silently overwritten. Fixing the sex-blind seed stands on its own merits whatever the report meant; everything else in that section waits on real words |
