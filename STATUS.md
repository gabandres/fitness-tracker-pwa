...
| **#107 Zero-Tap Sign-In — ANSWERED BY GOOGLE IN WRITING 2026-08-28; a dated critical path, and the date is 30 SEPT.** | **Two Google reviews, neither pollable, and ~31 days.** Play Developer Support confirmed all three questions the PESSIMISTIC way: **(1)** the exemption requires Block Store on the **Production track** — testing tracks do not qualify; **(2)** production access granted after 30 Sept does not qualify *even if the integration was live on a testing track before that date*; **(3)** the good news, and it is real: clearing 30 Sept keeps the Block Store implementation valid **indefinitely**, so no Restore Credentials migration ever. **THE COST OF MISSING IT** is the whole WebAuthn build this project dodged — a relying-party server as Cloud Functions, a public-key collection with rules, custom-token minting, a native `CredentialManager` module — by April 2027, in an app with no server by design. **THE CRITICAL PATH IS SERIAL AND EVERY STEP BUT THE LAST IS DONE:** production access (08-29) → health-apps declaration (08-29) → merge → **vc 40 built and device-verified** (§2) → **get it on a track and promoted to production before 30 Sept.** That last step is queued behind vc 37's production review — an `eas submit` now would cancel it. **Live consequence: the Android OTA channel is SHUT** (`ae526937…` → `d7ea3629…`) until vc 40 is on a track, so nothing reaches a real Android user by OTA meanwhile. **The release is the FIRST-EVER Android production release, under a deadline** — exactly when the `countryTargeting` trap gets skipped — so use `scripts/play-production-release.mjs`, never the console. **Still unverifiable here:** the restore leg fires only on a real Android-to-Android migration and needs two devices; 13 tests pin the guards, which is not the same claim. Design, alternatives and Google's full reply: ADR-0035 and issue #107. |
