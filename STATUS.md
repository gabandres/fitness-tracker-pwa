...
| **Activity correction is built end-to-end but DARK, blocked on copy** | **Wording, not math.** The continuous multiplier shipped 2026-08-20 (ADR-0024): `profile.activityMultiplier`, rules deployed and validated 1.0-2.5, floored at the **FAO/WHO/UNU free-living minimum PAL 1.40** — published, not fitted. On the owner's account it moves the anchor 2,530 -> **2,285** against a 2,385 benchmark (+6.1% -> **-4.2%**, inside the ±5% target) and collapses the `measured - anchor` gradient from **-263 to -18 kcal**, which is what 'agreement, not inequality' meant. **Nothing writes the field yet**: the only writer is the accept flow behind `FEATURES.activityTdeeInMeasured`, still `false`. The remaining blocker is that the card says *"switch to sedentary"* while accepting now stores 1.40 — a target between light and moderate. **Fix the copy to describe what it does, then unflag.** Do NOT hand-pick a bucket |
