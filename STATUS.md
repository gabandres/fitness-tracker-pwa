...
**TWO TDEE/onboarding fixes SHIPPED by OTA to BOTH platforms, 2026-09-04** (`5b4fb484`, `cf0953d4`; Android group `4a56c514`, iOS group `11edfb2f`, both from `eb1e3c2c`). **Both reach the public on next launch** — Android on Play production vc 44, iOS on build 63 now that 1.2.2 is `READY_FOR_SALE`; the first publish with neither channel shut. Device-verified on both phones (`apps/mobile/AGENTS.md` has the readings). **(1) An onboarding redo no longer overwrites a measured target with the wizard's heuristic seed.** **(2) The day still in progress is out of the intake mean** — and **this moves every user's target** (+30 kcal measured on the owner's account, +48 at 08:00; the old behaviour always erred toward a deeper deficit and was worst in the morning). Account and evidence: `CHANGELOG.md` 2026-09-04; device readings in `apps/mobile/AGENTS.md`; reasoning in `docs/training-backlog.md` §0–1.
