...
| **#76 — photo capture: items 1-4 BUILT and merged 2026-08-26, not yet delivered** | **A functions deploy, then an OTA per platform.** The description field, scale reading, repeat detection and the measured/estimated distinction are on `main`. **Item 5 (multi-image) stays gated** — `dailyQuota` still counts SCANS, and an unbounded 1.4x is still unbounded. **What the client half needs: nothing but an OTA** (pure JS, no fingerprint move). **What the server half needs: `firebase deploy --only functions`** — the note, the scale prompt and the `measured` flag all live in `analyze-photo.ts`, and until that lands the client sends a `note` nothing reads. **The seam worth not breaking:** `measured` is the one flag here that claims something about the physical world, so `resolveItem` drops it if `clampGrams` altered the number — a model claiming it read 6,200 g ships 5,000 g either way, and what it must not ship as is 5,000 g labelled a measurement. Rounding 180.4 to 180 keeps it; both directions are pinned. **Repeat detection abstains on purpose** (`packages/core/src/meal-repeat.ts`): a wrong "you logged this before" is worse than silence, because it is offered when the user is least likely to check. Three gates — containment, Dice >= 0.5, a shared word >= 4 chars; containment exists because a symmetric score alone offered `Chicken thigh` for a note saying `chicken breast`. **Still open and owner-only:** whether a matched repeat should log silently (it does not — it lands on Today via the normal add path) and whether the post-capture placement is right, which was settled by building it rather than arguing. ADR-0029 stays `proposed` until items 1-4 are DELIVERED, per its own definition of done. |
