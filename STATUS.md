...
| **Play alpha** | **vc 34** (1.2.0) — SDK 57, carries the preset/custom-food write fix, `status=completed`. Replaces vc 32, which CRLF normalization orphaned. **Device QA STARTED 2026-08-19** on the LG G6 (§3) — this row said `vc 33` until 2026-08-19, contradicting §2's `vc 34`; vc 34 is what the API and a real Play install both report |
