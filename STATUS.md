...
| **#76 — photo capture: description field, scale reading, repeat detection (ADR-0029, `proposed`, nothing built)** | **An owner decision on ordering; every stated blocker is now measured rather than assumed.** **(1) ADR-0029's risk was backwards.** It feared a front-of-pack phrase would *fail to resolve*; measured 2026-08-26, 20 branded phrases and a 20-phrase control both resolve at **100%** — **the resolver never abstains**, so the failure mode is a confident WRONG match nothing downstream can detect. Two classes were fixed and deployed the same day; the rest need head-noun awareness (own row). **Consequence: ship scale-reading, review UI and repeat detection — none depend on this — and do NOT auto-apply a macro from a description** until `resolvePhrase` can return `null`. `resolveItem` caps a *model*-sourced item at `confidence ≤ 0.5` while a *USDA*-sourced wrong match keeps full confidence, which is backwards for this failure mode. **(2) Both ADR defects are CORRECTED 2026-08-26, and one changes the answer.** The multi-image multiplier is **~1.4×, not 3×** — the benchmark's 1,009 tokens is the WHOLE call, not the image, and output tokens (two-thirds of the cost at $2.50/MTok) do not scale with image count. Real numbers: ~$0.0017/scan, free-tier worst case ~$0.15/user/month going to ~$0.21 with three images. **The gate still holds, on boundedness rather than magnitude:** `dailyQuota` counts SCANS, so **multi-image must not ship until it counts images.** The 768px-vs-1920px "contradiction" was **two different clients** (mobile 768 wide, web 1920 long edge), now named at both sites. Root cause of both: `analyze-photo.ts` named the wrong model in its own provider table for eight weeks, so the ADR read the owning file and the file was stale — fixed at source, and `usageMetadata` is now logged per call so the next pricing reads real traffic. Full write-up on issue #76. |
