...
**The 70% reliability cliff is removed — MERGED, IN NO BINARY AND NO OTA YET (2026-09-04).** `dailyTargets` no longer gates on `tdee.reliable`: under `'auto'` a measured estimate governs from the moment measured mode opens, damped by `confidence` exactly as before. `tdee-recalibration.ts` dropped the same gate so the explanation stays in step with the move. **Measured against all 42 PROD accounts first — it reaches exactly two**: `review@` 2,130 → 2,274 (+144) and one real user 1,640 → 1,500 (−140); everything else byte-identical. **The owner's account cannot demonstrate it** (cliff height 0 — its seed was deleted on 09-04), so `review@` is the account to verify on. **Known and accepted:** the real user's `ci95Tdee` is null, so their −140 lands with no recalibration notice. Reasoning, the rejected blend variant, and the artifact not to quote ("18 accounts move"): `docs/training-backlog.md` §0 and `CHANGELOG.md` 2026-09-04. **Still needed to ship: a `WHATS_NEW` decision and a device-verified OTA on both platforms.**
