...
| **"Not intuitive for women" — the WHOLE F-series is now built and shipped. Nothing here is blocking; what is left is iOS verification** | The report arrived in two pieces: one relayed sentence, then the actual ask — **"she needs some kind of tutorial or walkthrough of the app"**. **F0 the tour** (Android OTA 12–15, iOS 7). **F1/F2 the sex-blind calorie seed** (Android 17–18, iOS 9) — onboarding asks sex/height/age/activity and seeds from Mifflin-St Jeor; the old heuristic put a 180 lb 45-year-old woman's "lose fat" target **2 kcal/day ABOVE her own maintenance** against 0.69 lb/wk for a man. **F3–F7 shipped 2026-08-22** (Android OTA 24–25): **F3** body weight in kilograms anywhere the user reads or types it — `packages/core/src/body-weight-units.ts` is the seam and **storage stays in pounds**; the Settings row is honestly called *Units* now. **F4** the speed dial says *Search foods*, not *Manual entry*, and the sheet has a title. **F5** the + button no longer covers *Repeat yesterday*. **F6** a "?" on Today and Trends opening the same glossary component Train already had, nine terms in both locales. **F7** `CutPace` is unsigned and every consumer subtracted it, so a "gain" user was handed a DEFICIT once the estimator took over — `paceOffsetKcal` now reads `goalDirection`, and an absent direction keeps subtracting byte for byte. **Device-verified on Android** end to end, including 80.5 kg typed → **177.47 lb stored** read back out of PROD Firestore. Two layout defects were caught by the device and by nothing else, both from `flex: 1` meaning flexBasis 0 — see the OTA rows in `apps/mobile/AGENTS.md`. **What is open: iOS.** A Release simulator build + the Maestro suite (now 19 flows, `19-glossary` and `20-units-metric` added and registered) is the outstanding verification; the sim app had no `expo-channel-name` so it cannot take an OTA and needs a build. Suites: core 961, mobile 340, functions+rules 391. Do NOT design further work from the paraphrase; the precedent is Abdiel's calorie-goal item, where the summary said "no custom option" and the real defect was a number being silently overwritten |
