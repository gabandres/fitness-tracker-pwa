...
| **Photo scan — CODE COMPLETE, and three things have never been seen on a device** | **Owner, with a phone.** #76 is closed and ADR-0029 is `accepted`. What is unproven is not code: **no end-to-end scan has ever been driven through the app UI on either platform** (both photo pickers refuse automated selection), so the client putting a typed note or a photo array on the wire is unit-tested only — everything server-side was proven with `scripts/smoke-photo-scan.mjs` against PROD. **Multi-image de-duplication is improved, not solved**: the test images were a crop and an 8° rotation of one illustration and the last run still produced both "baked sweet potato" and "mashed potatoes". Judging it needs **2–3 real photos of one real meal**. Also never seen: **repeat detection has never fired** (needs a My Foods entry matching a typed note; it is deliberately quiet, so "never suggested anything" and "broken" look identical) and **`measured` has never appeared** (needs a legible scale AND unit in one photo). **The seam not to break:** `measured` drops if `clampGrams` altered the number, so a badge never names a weight the scale did not show. **Still owner-only:** whether a matched repeat should land on an editable draft rather than logging straight to Today. |
