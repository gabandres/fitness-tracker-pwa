...
| **TestFlight** | build **58** (1.2.0, SDK 57, runtime `b3c50e91…`) with *Public Beta Testers*, `VALID`. Build 57 is the older one on runtime `25e953e9…`. **No structured device QA** — and the QA that exists is weaker than it looks: the 17/17 iOS Maestro run captured Today against a ZERO-DATA account (`0 / 2,345 kcal`), so an empty hero ring was the *correct* rendering and the suite could not have caught the iOS ring bug (§3). This row said `57` until 2026-08-19, contradicting §2 |
