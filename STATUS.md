...
**Updated:** 2026-09-03 · **Owns:** current state only. Not history
...
| **TestFlight** | **build 63 / 1.2.2 (runtime `20a395de…`), uploaded 2026-09-03 via altool, attached to App Store 1.2.2 and submitted for review with release AFTER_APPROVAL** — build 62 (`3d6f4b68…`, the off-centre icon) was submitted first and its submission cancelled the same evening. Before that: **build 60 / 1.2.1**, runtime `7b347b0f…`, `VALID`, and on the EXTERNAL *Public Beta Testers* group (added 2026-08-24 — it was internal-only before that, and external testers had received nothing for six days). Group holds `60,58,57,53,49,47,45,7,4`. **Read the group's builds, never assume:** a build being `VALID` and on TestFlight does not mean any external tester can install it |
...
| **Play production** | **REJECTED 2026-09-03 (vc 37, Health Connect Minimum Scope: `StepsCadence/Steps`) — the reviewer had crashed the app on *Connect Health Connect* eleven minutes earlier. Nothing is live; the store URL 404s. The resubmission is **vc 44 / 1.2.2 — IN REVIEW since 2026-09-03 ~19:05 UTC-4** (production 100% + alpha + internal all on vc 44; Play Console reads *Changes in review*; built, verified and device-checked the same day (`CHANGELOG.md` 2026-09-03): no `READ_STEPS`, the permission delegate + the Android 14 privacy alias (the alias is ALSO what makes the grant succeed on Android 9), Zero-Tap #107, the Ember-on-Ink icon, dark splash, FCM. Runtime `68ea2dd3…` read from the `.aab`. **Nothing left to do until Google answers** — the Health apps declaration was re-saved without Steps once every track held the new bundle (Play derives the list from ACTIVE bundles: vc 39 on internal kept `READ_STEPS` detected until internal moved too), and the three changes were sent from Publishing overview. Read the outcome at Policy status / the Inbox; the store URL turning 200 is the only user-visible truth. Landing on production also clears the **#107 Block Store deadline (30 Sept)**. The 128-not-145 and no-API-for-review-status facts from 08-29 still hold; vc 41 and vc 42 are uploaded bundles on no track, superseded |
...
|---|---|---|---|
| Android | `68ea2dd3…` | **vc 37** ships `ae526937…` (read from the `.aab`); vc 44 (`68ea2dd3…`) is in Play review | **SHUT** until vc 44 is live |
...
**Everything merged is in a binary — vc 44 (Android) and build 63 (iOS), both built 2026-09-03 — and both are with their store's reviewers.** Android: vc 44 in Play review on production + alpha. iOS: 1.2.2 / build 63 submitted for App Store review, release AFTER_APPROVAL. What rode in: Zero-Tap Sign-In (#107), Milestones (#108–#110), the Trends stub labels and water card (#115), the welcome intro, the onboarding reminders + first-log steps, the funnel events, the lapsed nudges, the log stopwatch, the rest-timer change — every Android OTA 88–109 that only ever reached the LG — plus the Ember-on-Ink icon, the dark splash, FCM (token registration verified in PROD), the iOS push entitlement, and the two Health Connect fixes. `CHANGELOG.md` 2026-09-03 has the evidence; the AGENTS.md rows for vc 44 and build 63 have the artifact reads (vc 41–43 and build 62 were the same evening's iterations: 41 lacked the Android 14 alias, 42 carried version 1.2.1, 43 had the off-centre icon).
...
| **Resolver qualifier mis-matches — CLOSED on `main` 2026-08-31, NOT DEPLOYED** | **One command when the owner wants it: `firebase deploy --only functions:analyzePhoto`.** All four #76 classes are fixed; the last (`Coffee, Cuban` over `Coffee, brewed`) was a missing signal, not a tuning problem — three penalties in `photo-resolve.ts` (unasked-for Title-case variety −5, photo context cancels the `nfs` −25, `skin eaten` −5), never filters, `searchFoods`/`getFoodDetail` untouched. 117-phrase corpus: 21 changes, 0 regressions; six tests pin it (the −25 sizing regressed `walnuts`, hence 5). **Deliberate non-change:** `greek yogurt` → whole milk stays (touching `PRODUCT_QUALIFIERS` reopens the `nonfat` fix). Full write-up: `CHANGELOG.md` 2026-08-31. |
...
| **Push notification when an OTA publishes** (#112, #114) — **everything is in place; the first real send is the proof** | **One command, owner-run (the agent classifier blocks it): `node scripts/announce-ota.mjs --platform android --message test`.** Both credential gaps closed 2026-09-03 evening: the FCM V1 key is on EAS (`fit.ignia.app` → Service credentials, key id `9c44c3262c1d…`, uploaded via expo.dev — `file_upload` drives its picker, the CLI is not needed), and the APNs key **already existed** — `G27PU9VMS9`, team `AE6TTXW92K`, on EAS since Jul 7 2026, so the "eas build did not create one" line was wrong. Token registration is proven on Android (QA profile `expoPushToken`). **Store declarations were amended the same evening because vc 44 stores that token:** Play Data safety now declares *Device or other IDs* (collected, not shared, optional, app functionality) — sent for review, which **restarted the vc 44 review** (owner's call, 09-03); ASC App Privacy now lists *Device ID* (app functionality, linked, no tracking), published. |
