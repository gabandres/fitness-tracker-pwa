...
**TWO mobile fixes, merged 2026-08-27 in `838d79e6`, in NO binary and in no
OTA.** Both are JS-only and both channels are open — the Android tree
fingerprints `ae526937…`, matching vc 37 exactly, and the export passes the
perf budget at +2.58% — so **one `eas update` per platform delivers them.** They
were not published only because OTA publishing was unavailable in that session.

| Fix | Signature it closes |
|---|---|
| `app-update.ts` fallback rejection | `IGNIA-MOBILE-8` — the *update available* button could fail silently AND report a crash |
| HealthKit `Code=5` swallowed | `IGNIA-MOBILE-H` — an expected OS state re-thrown on every foreground |

**The first one is the more urgent of the two, for a reason that compounds:**
its audience is by definition on an old build, and on Android that is often an
orphan runtime no OTA reaches — so for those users that button is the only
route forward, and it is the one that breaks.

Publish with the two commands in `build-android` Step 2 (Android from Windows,
iOS from `ignia-mac`), then record both on the vc 37 rows in
`apps/mobile/AGENTS.md` and empty this section again.
...
| **DSA trader status — the last 30 iOS territories. STEP 2 IS ONE COMMAND; STEP 1 IS THE OWNER, and that was CONFIRMED not assumed** | **Owner, and only for the declaration.** ASC API, re-read 2026-08-27: **145 of 175 available, 30 blocked — the 30 being the EU 27 (all of them) plus GBR, ISL, NOR.** EU distribution triggers Apple's trader declaration, which **publishes** the trader's name, address, phone and email on every EU product page; values are in `CLAUDE.local.md` — the Sheridan address and `+1 307 201 8420`, never the personal number. **Attempted 2026-08-27 and it is not agent-reachable:** `appstoreconnect.apple.com` returns `authResult=FAILED` in this Chrome profile, and completing it needs an interactive Apple ID sign-in. Set it at *ASC → Business → Trader Status* on the **LLC** entity (`94754902`). **Then step 2 is `node scripts/asc-eu-territories.mjs --commit --trader-status-is-live`** — preview by default, and `--commit` **refuses** without that flag because **the wrong order is destructive**: flip the territories while the entity still declares "not a trader" and the app is REMOVED from all 27 EU territories, worse than today and a re-review to leave. No API reports the declaration, so the flag is a person asserting it. `availableInNewTerritories` stays **FALSE** and the script never touches it. **The API shape that cost two failed attempts is encoded rather than remembered:** `POST /v2/appAvailabilities` is a CREATE and 409s post-launch; the mutation is `PATCH /v1/territoryAvailabilities/{id}`, one at a time, with opaque ids that must be READ, not constructed. |
| **#107 Zero-Tap Sign-In — BUILT on `feat/zero-tap-block-store`, NOT merged. A 30 Sept deadline decides which implementation counts.** | **Play production access, and one written answer from Play support.** Play names the **Restore Credentials API**, which is WebAuthn end to end and whose own guide makes *"set up a relying party server"* a prerequisite — this project has no app server by design. **Google accepts a Block Store integration instead, "only if completed and in production on or before September 30, 2026"**, and that is what is built: Android-only, no server, no new secret, no new rules collection. **ADR-0035 and issue #107 carry the design, the measurements and the alternatives — not repeated here.** **Why it is on a branch:** merging moves the Android fingerprint `ae526937…` → `5facf778…`, which shuts the Android OTA channel until vc 40 is on a track, and OTAs are currently the delivery mechanism. `modules/` not `app.json` was deliberate and measured — via `app.json` it would have moved iOS too and shut that channel while build 60 is public. **The restore leg is UNVERIFIED and cannot be verified here** — it fires only on a real Android-to-Android migration, needing two devices; 13 tests pin the guards, which is not the same claim. **Play support ticket FILED 2026-08-27** from the ORG account, topic *Android vitals and technical quality* / *Restore credentials*, app-scoped; **Case ID was pending at filing — read it back at Play Console → Help → Your support tickets**, reply due within 2 business days. It asks whether "in production" means the production track, whether a testing track before 30 Sept counts, and whether a qualifying Block Store integration stays compliant. **Play's AI pre-triage answered all three pessimistically and is NOT authoritative** (its own banner says so): production track specifically; testing does not count; access granted after 30 Sept voids the exception even if the bundle was on a testing track first. **Plan against that reading until support contradicts it in writing** — it makes 30 Sept a production-track deadline this project does not control, since access was only applied for on 26 Aug. If the date is missed the branch still delivers the UX and nothing is wasted. |
