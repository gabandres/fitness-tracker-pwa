...
**Updated:** 2026-09-01 · **Owns:** current state only. Not history
...
| 3 | **Instrument the two deciding numbers** — seconds-per-log by method, D7 split by dominant logging method, `time_to_first_log` from `firstEntryAt − createdAt`. | **SERVER HALF SHIPPED 2026-09-02** (`4b8aef99`): rules + `hourlyTasks` deployed, the retention pass writes `byMethod`, `timeToFirstLog`, `secsPerLog` to `config/retention`, the admin Overview prints them (`CHANGELOG.md`). **CLIENT HALF — the `log_secs` stopwatch — is on `main` and NOT PUBLISHED**: the OTA invocation was denied by the session's permission classifier, so the owner runs the Android publish from this box and the iOS one from `ignia-mac` (commands in `.claude/skills/build-android/SKILL.md` step 2; gate `d7ea3629…` = vc 40 re-read today, export +3.58% raw within budget). Until it lands `secsPerLog` stays `null` — the denominator only counts timed logs, by design. `timeToFirstLog` and `byMethod` need no client and are live now — **first PROD read 2026-09-02, 11 activated: time to first log median 1 h 32 m, p75 4 h 49 m, 17% inside five minutes (n=12)** — that is the number lever 1 has to move; the split is 7 `unknown` (activated before analytics shipped 08-12), 3 search, 1 photo, so the method comparison is months away. |
