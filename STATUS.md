...
| 9 | **Count the reminders opt-in** (`reminders_on` usage event) — every notification lever reaches only these users, and nothing could say how many. | **Rules DEPLOYED + iOS SHIPPED 2026-09-10; Android WAITS with lever 8.** First read: the 09-11 daily pass onward, iOS-only until Android ships. If low, the next lever is asking for reminders AFTER the first log lands, not before it. |
