...
| — | Play launch — **Google Sign-In broke a second time: Play App Signing is enrolled in "Quantum-ready (beta)", which means THREE app-signing certificates, not one** | **FIXED 2026-08-05, server-side, no build — awaiting confirmation from a tester.** Sentry caught `DEVELOPER_ERROR` at `signInWithGoogle.picker` on 2026-08-05 03:00:17Z from a real Play split-APK install of **vc 6** (Samsung SM-S942U, Android 16, `is_split_apks: true`), **31.5 h after** the 08-03 SHA fix (audit log: `CreateShaCertificate` at 2026-08-03T19:32:13Z), so propagation delay is ruled out. The tester then signed in with a password 2 minutes later (Firebase Auth, 03:02:15Z) — and **no `google.com` account has been created or signed in from a Play install, ever**; the last Google sign-in is 2026-08-01, a day before any AAB existed. Cause: Play Console → *Protected with Play → App signing* shows the app signing key as **`In use · Quantum-ready (beta)`, with TWO fingerprint columns — Classical AND Post-quantum cryptography** — plus a **Previous app signing keys** entry (first used 1 Aug 2026 01:14). Google's own instruction for hybrid signing is to register **all three** with every API provider ([Use Play App Signing](https://support.google.com/googleplay/android-developer/answer/9842756)); only the classical one was registered, so a modern device that verifies the app against the PQC certificate presents a SHA-1 Google has never seen and Play Services rejects the call before Firebase Auth is ever reached. **Registered 2026-08-05** — PQC `1106505210026f5c9c7fbba13c2b058375910ff4` (→ OAuth client `…vearntjrg4dkj7…`) and previous key `1483ddc326f972896e03d8db45ba4edd1d621da4` (→ `…h4j12fvoic0d3…`); confirmed present in `firebase apps:sdkconfig ANDROID`. **The console page hides these behind copy-to-clipboard chips — the values are not selectable text, so reading only the one visible SHA-1 is exactly how this was missed twice.** Re-check after any key change with the command in `CLAUDE.local.md` | `npx firebase apps:android:sha:list 1:647810616435:android:a6f4c5f9e200b3332c2e06 --project fitness-tracker-gb-1775407101` — must list **5** hashes |
