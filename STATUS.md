...
| **Trends consolidation + dismissible stubs are LIVE; "My View" is CLOSED** | **Nothing.** Trends went six elements -> **four** on 2026-08-26: a **Weekly** panel (This week ⇄ Budget) and a **Habits** panel (Sleep ⇄ Fasting), with the tab choice remembered per device in AsyncStorage. Live on both platforms (Android OTA 71–72, iOS OTA 39) and device-verified on the LG G6 in both themes, including the choice surviving a cold start. **This is ADR-0034's own ordering — consolidation before configuration — and it moved the reopening trigger FURTHER away, not closer**: the trigger was >8 elements or three users asking, and the count is now 4 with 0 users. **#103 was reopened by the owner and then answered rather than built.** The question was what four elements would actually get hidden, and the answer was: nothing except the rows that say "you have no data". Those are now dismissible (Android OTA 73–74, iOS OTA 40), device-verified including the card returning when evidence arrives. The catalogue was not built and the trigger stands. **The seam not to break:** the tab cache is AsyncStorage and must stay that way — option C stores hidden ids on the PROFILE, needing a `firestore.rules` deploy that is cross-frontend, and getting it wrong rejects the frozen web's profile writes too. |
