...
| — | Play launch — **upload the AAB to a closed track** | **owner, in the console — and this first one has to be by hand regardless.** Play does not accept an app's *first* upload through the Developer API; the console is the only path until one release exists. Must be **closed** testing, not internal — closed is what starts the 14-day clock. Artifact: `~/Downloads/ignia-1.1.0-vc3.aab`, or the EAS build page |
