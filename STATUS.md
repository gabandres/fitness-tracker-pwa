...
| **Transactional mail was still coming from a shared sandbox domain — HALF FIXED 2026-08-14** | **Resend half: FIXED AND DEPLOYED** (`sendWelcomeEmail`, `sendPasswordReset`, `hourlyTasks`). `mail.ignia.fit` has been `status: verified` / `sending: enabled` in Resend since **2026-07-24**, with DKIM + SPF live in Cloudflare and DMARC `p=none` on the apex — and `MACROLOG_EMAIL_FROM` was never set on the runtime, so for **three weeks** every welcome, password-reset and weekly-digest mail shipped from `onboarding@resend.dev`, Resend's shared sandbox, unaligned with `ignia.fit` and carrying every other developer's reputation. `docs/email-deliverability.md` said "awaiting Resend's verification poll" the entire time, which reads as *blocked* rather than *stale*. Fixed at the **tracked default** (`FROM_FALLBACK` in `functions/src/resend-client.ts`), not in a `.env` — a fourth place to forget is what caused this. **Reply-To deliberately stays a personal Gmail**: `hello@ignia.fit` has no MX and would black-hole every reply. **STILL OPEN — the half that actually junked a user's mail: Firebase Auth's OWN email verification does not go through Resend at all.** Both clients call `sendEmailVerification` directly (`src/app/services/auth.service.ts:256,337`, `apps/mobile/src/lib/auth.tsx:719,782`), so it ships from `noreply@fitness-tracker-gb-1775407101.firebaseapp.com` with `method: DEFAULT`, `customDomainState: NOT_STARTED`, and an action link on `firebaseapp.com`. **No DNS record on `ignia.fit` can fix that** — alignment compares the `From:` domain. **Email verification is the signup wall (§6), so this is a failed activation, not a missed newsletter.** The fix is Auth → custom SMTP at `smtp.resend.com:587` (user `resend`, password = the `RESEND_API_KEY` secret, sender `hello@mail.ignia.fit`) + action URL `https://ignia.fit/__/auth/action`, which already serves 200 and is already an authorized domain. Runbook + the API call: `docs/email-deliverability.md` §6 | `node scripts/resend-domain-setup.mjs --verify` · Gmail *Show original* → `dkim=pass header.d=mail.ignia.fit` |
