...
**Two follow-ups the device pass turned up, one MERGED-NOT-PUBLISHED and one OPEN.** (1) **`50e6c844` — the Today card showed *"held steady"* next to *"just recalibrated"***; the recalibration line only began co-occurring with `provisional` because of the commit above, so the copy had never been wrong in front of anyone before. Fixed in all three locales, **not yet published** — it needs the next OTA. (2) **OPEN, and not caused by that OTA: the owner's iPhone landed on `sign-in.tsx` after taking the update.** `git diff eb1e3c2c..11e75966` touches no auth, routing, session or storage file, and the OnePlus took the same bundle two minutes earlier and came up signed in. It is the cold-start auth-rehydration flash `sessionPresumed` (#83) exists to suppress — `auth.tsx:679` records `onAuthStateChanged` at ~771 ms to ~8,690 ms. **An OTA apply is exactly when it is most likely to be seen, so it will recur on every publish until someone widens that guard.**
...
| **The exposed `Fitness Tracker Gemini (client)` key — delete it; Google already disabled it** | **Owner, one console click, no urgency.** It is genuinely exposed: it sat in `src/environments/environment.ts` in this PUBLIC repo until `15da1e2b` (2026-07-01) and **the history was never rewritten**. **Verified unbound and dead, 2026-09-04.** The one-call Cloud Run scan shows `analyzephoto` / `consultationstream` / `generateweeklyreport` binding Secret Manager `GEMINI_API_KEY`, whose value SHA-256-matches the *server* key (`79a1ffc3…`), not this one (`c8357f5f…`); zero tracked files reference it. A live probe returns `403` — *"Your API key was reported as leaked"* — with or without an allowed `Referer`. **So there is NO billing exposure through it**, reversing the earlier framing that the card and auto-reload raised the stakes. Unrelated and unexplained: the Gemini API took **241 403s in 30 days, 4–23 daily**, from callers that are not our functions; `credential_id` is not a label on that metric, so it cannot be attributed to this key. |
