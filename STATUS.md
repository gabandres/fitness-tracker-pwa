...
- **The Apple Watch complication and its transport** — the watch app
  ...
  `WCSession.updateApplicationContext`. **On TestFlight in build 16** (`VALID`, 2026-08-03) and **tested on a real Apple Watch 2026-08-04. Result: the transport works, the auto-refresh does not.** The mirror screen and the complication both render real kcal-left and protein numbers, which proves the whole delivery chain plus the App Group, the entitlement, the shared decoder and `_shared` at runtime. **But the face does not move after a meal is logged** — it only catches up when the watch app is launched fresh. Two fixes are committed and, **as of 2026-08-06, ARE in TestFlight build 19 — so this is now the cheapest high-value verification on the board: install build 19 on the paired watch, log a meal, and see whether the face moves without launching the watch app**: `8cc2ba39` (the complication's `.atEnd` timeline meant one render decided the whole day — confirmed by remove/re-add restoring it) and `360662eb` (**the actual cure**: the `.backgroundTask(.watchConnectivity)` handler called `refresh()`, which re-reads but never *stores*, so no reload was ever requested — and it returned before activation completed). Build 19 carries both; until it is confirmed on hardware, keep telling testers the workaround — log, then open the watch app. What is still unproven beyond that: the layouts at 40mm/46mm in both locales (#46, needs a simulator). What *was* verified locally: `tsc` clean, both `expo-target.config.js`
