...
| Work | Blocked on |
|---|---|
| **Expo SDK 57 — merged and on both test tracks, but NO ONE HAS RUN iOS ON A DEVICE** | **iOS device QA. That is the only part still open.** Android now has a host and its own row below. Green everywhere it can be: `tsc`, 26/26 jest suites, the Metro gate, all four buildable units, `expo-doctor` 20/21, artifact verifier 0 on both binaries — none of which exercise a device. The checks that matter and cannot be automated are a normal log/read pass on each tab and the hero-ring fix; the Android half of that ran 2026-08-19 and the rings render correctly there, so the ring bug is iOS-only |
...
| **Transfer of operations to Bermudez Systems LLC (WY)** | **Owner console steps, in this order**: ① ~~EIN~~ **DONE 2026-08-19** (number + CP 575 location in `CLAUDE.local.md`) → ② ~~D-U-N-S~~ **DONE 2026-08-19** — Apple issued it same-day (number in `CLAUDE.local.md`) → ③ Apple Developer individual→organization **conversion** of the existing team — NEVER a fresh org enrollment, which strands the app on the old team (support request; Team ID/apps/reviews survive, seller name changes) → ④ ~~deactivate the 3 `fit.ignia.tip.*` consumables in ASC~~ **DONE** — all three read `DEVELOPER_REMOVED_FROM_SALE` from the ASC API 2026-08-19 (`inAppPurchasesV2`); that state is reversible, so re-enabling tips is a flip, not a re-create → ⑤ ~~LLC bank account~~ **DONE 2026-08-19 — Relay approved** (details in `CLAUDE.local.md`) → ⑥ ~~new Play org account ($25)~~ **DONE** and ~~Google org verification~~ **GREEN, verified from the console 2026-08-20**. **⑦ THE APP TRANSFER IS THE LIVE ACTION and is now a five-minute owner task.** Org verification was **confirmed from the console 2026-08-20** (not owner-reported): green checks on website, contact email and phone, no banner, zero apps. The form is **self-serve, not a support ticket** — sign in as the *personal* account (Play Console `/u/1/`; its developer-account ID is in `CLAUDE.local.md`) → **Settings → App transfers → Transfer apps**. It asks four things plus the app picker, and wants **fewer** values than assumed: there is **no source-account transaction ID field**. **Filed 2026-08-20 and REJECTED at the last step on one value.** Everything else validated live: the developer ID resolved to *Bermudez Systems LLC* via Find, both dropdowns took, Ignia was picked, the review dialog rendered correctly and the agreement was accepted — then *Agree and transfer* returned *"the transaction ID or account verification code you've entered doesn't belong to the target developer"*. **No transfer request was created**; the list is still empty, so this is a retry, not a cleanup. The value recorded for this field is the **payments order number off the purchase receipt** (the `PDS.` one), which is not what the field wants. Google names the two places the right value lives — the org owner's inbox, subject *'developer registration fee'* or *'Your new Play Console account verification code'*. **That one lookup in gabriel@bermudezsystems.com is the whole remaining blocker**; full detail and the exact other three values are in `CLAUDE.local.md`. Org accounts are exempt from the 12-tester/14-day production gate, so completing this also removes the Play production blocker above. Separate Google deadline found 2026-08-19: **all Play apps + signing keys must complete "Android developer verification" by Sep 30, 2026** — after the transfer that is the ORG account's task for `fit.ignia.app`; do NOT assume the personal account's Jul-8 identity check carries over. **Donation intake stays paused** (`FEATURES.tips=false` both platforms, `/tip`→`/support`) until payouts land in the LLC's bank account; do NOT re-enable tips or ship Pro before then. PR foreign registration deliberately skipped: zero revenue + interstate-commerce exemptions; worst case is back-fees, not veil loss |
