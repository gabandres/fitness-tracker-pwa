...
| **LLC migration — GCP side DONE 2026-08-30; the Cloudflare domain move is the one piece left** | **Owner, runbook `docs/DEV_ENVIRONMENT.md` §5.1.** Done and verified: org account is project Owner (§5.0, invite accepted); billing account **`01916B-2927E2-E01DC7` "Bermudez Systems LLC"** (LLC payments profile, Visa ••5311 by the owner's call, $300 trial credit to 2026-11-29, upgraded to pay-as-you-go so nothing expires); Ignia moved onto it; billing export re-enabled there (Ignia/`billing`); the old account's export disabled so only ONE `gcp_billing_export_v1_*` table family lands in the dataset `cost-model.ts` reads. Two leftovers: the Cloudflare/registrar move of `ignia.fit` (§5.1 — export the zone FIRST), and granting the personal Gmail Billing Account Admin on the new account (1 min, recovery symmetry). |
