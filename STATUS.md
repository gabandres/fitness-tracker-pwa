...
| **Stretching / mobility in Train — SHIPPED to both platforms 2026-08-27, with one known gap** | **Android OTA 78 on vc 37 (`01a044ec…`) and iOS OTA 44 on build 60 (`01a044fd…`), both from `9e59f6e0`.** Runtimes `ae526937…` / `7b347b0f…`, each matching the live binary, so both reach testers on next launch. ADR-0028 implemented in full; per-OTA detail and the device evidence are on the vc 37 row in `apps/mobile/AGENTS.md`. **The remaining work is one gap the owner spotted after the ship, and it is real:** `TemplateEditorModal.tsx:325` hardcodes three `working` sets when an exercise is added, whatever log style was picked. So a stretch created **by hand** comes out `logStyle: 'time'` with `kind: 'working'` — which means it can still set a `maxDurationSec` PR, and the dose note never fires on it. That is exactly the outcome ADR-0028 decision 1 exists to prevent, reached by the hand-authoring path instead of the seeded one. **The seeded path is correct and verified in prod Firestore** (eight rows, `progression: null`, 14 sets all `{kind:'mobility',durationSec:45}`), so this is a defaulting bug, not a model error — no ADR revision needed. **The fix is one line plus a decision**: default a `time` exercise's sets to `mobility`, or leave `working` and surface the kind at creation. Leaning to neither yet — a plank is also `time` and is genuinely a working set, so defaulting on log style alone would mis-tag it. |
