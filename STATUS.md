...
**Onboarding funnel step events (2026-08-31) — merged, rules DEPLOYED, delivered nowhere yet.** Three markers (`onboarding_start`, `onboarding_step_body`, `onboarding_step_plan`) locate where the measured 50% signup→complete drop happens; `onboarding_start` also counts federated arrivals, which `signup` deliberately never did. Rules accept the fields in prod already, so the client half safely rides the next OTA pair — publish before Play production opens the traffic tap or the launch cohort's funnel is unreadable.
