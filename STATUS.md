...
| **"Not intuitive for women" — BOTH halves now built. F0 (tutorial) is live; F1/F2 (the sex-blind calorie seed) is on `main` and needs a publish** | The report arrived in two pieces: one relayed sentence, then the actual ask — **"she needs some kind of tutorial or walkthrough of the app"**. **F0 SHIPPED 2026-08-22**: a six-card guided tour (`apps/mobile/src/app/tour.tsx`), auto-opening once per device, replayable from Settings → How Ignia works. Live on Android (OTA numbers 12–15, vc 37) and iOS (OTA number 7, build 60); device-verified on Android, UNVERIFIED on iOS as usual. **F1/F2 BUILT 2026-08-22, not yet published** — onboarding now asks sex/height/age/activity and seeds the target with the app's own Mifflin-St Jeor path (`packages/core/src/onboarding-seed.ts`) instead of weight × {11|14|17}. The defect it closes: at 180 lb / 45 y the old "lose fat" target of 1,980 kcal sat **2 kcal/day ABOVE** a woman's estimated maintenance (0.00 lb/wk, forever) against 0.69 for a man — a **2.4×** gap driven entirely by the two inputs the heuristic could not take. She now gets 1,500. Both new steps are skippable and a skipped set writes **none** of the five fields, because the rules validate them as a group. **No `firestore.rules` change was needed** — proven by three new cases in the rules spec, not assumed. Suites: core 940, mobile 321, functions+rules 391, all green. **Still open: F3–F7 in `UX_AUDIT.md`.** F5 is a ten-minute layout fix, F4 is one string, F3/F6 are scope. **F7 is new and is a correctness defect**: `CutPace` is unsigned and every consumer subtracts it, so a "gain" user is handed a deficit once the estimator takes over; F1 works around it by persisting pace 0 for gain, a real fix needs a signed pace or a goal-direction-aware target. All of it is JS-only — nothing here touches the Apple migration precondition. Do NOT design the rest from the paraphrase; the precedent is Abdiel's calorie-goal item, where the summary said "no custom option" and the real defect was a number being silently overwritten |
