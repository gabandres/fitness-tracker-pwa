...
| **"Not intuitive for women" — BOTH halves SHIPPED 2026-08-22. What is left is F3–F7, none of it blocking** | The report arrived in two pieces: one relayed sentence, then the actual ask — **"she needs some kind of tutorial or walkthrough of the app"**. **F0 (the tour)**: six cards, auto-opening once per device, replayable from Settings → How Ignia works. Android OTA numbers 12–15 (vc 37), iOS number 7 (build 60). **F1/F2 (the sex-blind calorie seed)**: onboarding now asks sex/height/age/activity in two skippable steps and seeds the target with the app's own Mifflin-St Jeor path (`packages/core/src/onboarding-seed.ts`) instead of weight × {11|14|17}. The defect it closes: at 180 lb / 45 y the old "lose fat" target of 1,980 kcal sat **2 kcal/day ABOVE** a woman's estimated maintenance — 0.00 lb/wk, forever — against 0.69 for a man, a **2.4×** gap driven entirely by the two inputs the heuristic could not take. Android OTA numbers 17–18 (vc 37), iOS number 9 (build 60). **Device-verified on Android end to end INCLUDING the PROD Firestore write** — the five profile fields stored, the account's existing 0.8 lb/wk pace preserved rather than overwritten, `targetsRefinedAt` stamped, and the write accepted with **no `firestore.rules` change**, which three new rules-spec cases prove rather than assume. UNVERIFIED on iOS as usual. **Open: F3–F7 in `UX_AUDIT.md`.** F5 is a ten-minute layout fix, F4 is one string, F3/F6 are scope. **F7 is new and is a correctness defect of the same family as F1**: `CutPace` is unsigned and every consumer subtracts it, so a "gain" user is handed a deficit once the estimator takes over. F1 works around it by persisting pace 0 for gain; a real fix needs a signed pace or a goal-direction-aware target, and it touches `packages/core` shared with the frozen web app. All of it is JS-only — nothing here touches the Apple migration precondition. Do NOT design the rest from the paraphrase; the precedent is Abdiel's calorie-goal item, where the summary said "no custom option" and the real defect was a number being silently overwritten |
