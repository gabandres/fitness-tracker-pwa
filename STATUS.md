...
| **Play production** | **LIVE — vc 37 / 1.2.1, `completed` (100%), released 2026-08-29.** The first Android production release ever. Read from the androidpublisher tracks API, not the Console: `countryTargeting` is `null`, which is correct here — it INHERITS the track, and the track reads **128 countries, restOfWorld=false**. Production users receive every OTA already published, because vc 37 keeps runtime `ae526937…`. **128, not 145:** Play's picker offers 176 territories and does not offer 17 that iOS ships to (`NOT_OFFERED_BY_PLAY` in the release script) — a platform limit, not drift. The EU 27 + GB/IS/NO remain deliberately absent pending Play's own DSA declaration (§3) |
