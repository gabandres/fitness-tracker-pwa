...
**One exception, and it needs the owner: PR #120 is merged and unpublished.**
`first-scan` (#109) and the Health Connect parse fix are on `main` and in no
binary or update. The Android channel is OPEN (gate run on Windows:
`68ea2dd3…` = vc 44, live on production), the Metro export measured +3.73% raw
within budget, and **`eas update` is refused by the session's permission
classifier** — the same block as OTA 100 and 108–109. One command, from a
PowerShell window in `apps/mobile`:
`npx eas update --platform android --branch production --environment production --message "first-scan (#109) + Health Connect instant parse"`.
iOS waits on 1.2.2 being `READY_FOR_SALE` regardless. **The Health Connect half
is a live production bug**: `IGNIA-MOBILE-S` fires once per app foreground for
every Android user with Health Connect connected, and takes Android's whole
active-energy *and* cardio import down with it (`CHANGELOG.md` 2026-09-04).
