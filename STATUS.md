...
| **Activity multiplier is a 5-bucket ladder, and it now feeds measured mode** | **A design decision (Step 3), then unflagging Step 2.** `measuredConfidence` (2026-08-19) blends a thin measured estimate toward the Mifflin x activity anchor, so `profile.activityLevel` now moves the target for **measured-mode** users — it never did before. The correction card that fixes the bucket is built and works but is **dark** (`FEATURES.activityTdeeInMeasured = false`), because on the owner's account it suggests `sedentary` (anchor 1,958, **-17.9%** vs the 97-day energy balance of 2,385) — worse than the stored `moderate` (+6.0%). Two causes: the ladder cannot express the implied 1.278 (rungs 0.175 apart = +/-285 kcal here), and Apple `activeKcal` understates NEAT so the implied multiplier is biased low before snapping. **Acceptance for unflagging: the anchor must land BELOW the measured value**, which is what makes `d(damped)/dc` positive so better logging raises the estimate. Do NOT hand-pick a bucket — swapping one wrong constant for another while removing a compensating error makes it worse |
