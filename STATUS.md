...
| **Stretching / mobility in Train — BUILT and committed, in no binary yet** | **An OTA. Not published — it has never run on a device, and this is new UI plus new copy in three languages rather than a bug fix.** Implemented to ADR-0028 (`accepted` 2026-08-26) exactly: `SetKind` gains `'mobility'` in all **FOUR** mirrored sites — the fourth is `template-editor.component.ts`'s `SET_KINDS`, which compiles cleanly when missed and silently never offers the kind — and `isWorkingSet` excludes it, which is the single clause standing between a 60-second pre-lift hold and a `maxDurationSec` PR. `packages/core/src/mobility.ts` is the dose guardrail: it flags mobility prescribed over **60 s before the first working exercise**, keys on position and duration ONLY (there is no static/dynamic distinction in the model and amendment 1A decided not to invent one), and returns **nothing at all** for a mobility-only template, since with no lift to follow there is no `pre` position to protect (amendment 1B). It warns, it never caps. Eight seeded movements ship with es-PR and pt-BR, reachable through the existing clone path as a `Mobility Reset` starter template — the ADR names `STARTER_TEMPLATES` as the sanctioned home for a prescribed sequence, so this needed no new picker. `SeedExercise` gained `logStyle`, because both clone paths hardcoded `'weight-reps'` and a timed hold logged as load x reps has nowhere to put the hold. **One thing the ADR got wrong and the tests caught:** it reasoned `sessionVolume` was already safe "because a hold carries neither weight nor reps" — true of a bodyweight stretch, false of a LOADED one, which a user can type. `sessionVolume` had no kind gate at all, so mobility now skips it; warm-ups and drops still count toward tonnage exactly as before. **No `firestore.rules` change, no new collection, and the fingerprint is MEASURED not assumed** — `npx expo-updates fingerprint:generate --platform android` returns `ae526937893adb7f7349321b05caf2732da9658b`, byte-identical to `main`'s, so the Android OTA channel is intact. Web gets the render-safe half only (ADR-0022): the kind is offered and labelled in both locales, but the web editor has no duration input, so the dose note is mobile-only where templates are actually authored. core 1382 green, mobile 513 green, web 206 green, all four units build. |
