...
| **Stretching / mobility in Train — SHIPPED and CLOSED 2026-08-27** | **Nothing. Two OTA pairs, both platforms, both device-verified.** OTA 78/44 shipped ADR-0028 (`97a2a19c`); OTA 79/45 closed the one gap it shipped with (`67193489`). Runtimes `ae526937…` / `7b347b0f…` throughout — the fingerprint never moved, so every publish reached the live binaries. Per-OTA detail and the device evidence live on the vc 37 rows in `apps/mobile/AGENTS.md`. **The gap is worth remembering because of how it was found:** the owner read the shipped result and asked how you tell an exercise is a stretch. The answer was that you could not, and the cause was worse than a missing label — every creation path scaffolded `working` sets regardless of log style, so a hand-typed stretch could take a duration PR and never trip the dose guardrail. It had **two doors** (template editor and live session), and fixing one would have left the other to be rediscovered. **No ADR revision was needed and none was made.** The model was right; the defaulting was wrong. Amendment 1A's refusal to classify `Exercise` still stands — the fourth chip resolves to a `logStyle` plus a `SetKind` and stores no new field. |
