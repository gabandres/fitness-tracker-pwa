...
| **#71 — stretching / mobility in Train (ADR-0028, `proposed`, nothing built)** | **A decision, and the research already changed the feature.** Static stretching BEFORE lifting costs measurable strength — Simic et al., 104 studies: maximal strength **−5.4%**, power **−1.9%**, dose-dependent (smallest ≤45 s; past ~60 s the deficit climbs fast). The Cochrane review (Herbert & de Noronha, 12 studies, one field trial n=**2,377**) finds stretching before, after or both gives **no clinically important reduction in soreness** — about **4 points on 100**. So the obvious version of this feature ships the one claim every fitness app makes and the evidence contradicts, which for a product positioned on measured honesty is a copy decision as much as a product one. What IS supported pre-training is a structured dynamic warm-up (RAMP — Jeffreys 2006), and **Ignia already implements the P**: `packages/core/src/warmup.ts` is the load ramp. That is also the naming trap — `warmup` already means the load ramp, so shipping stretching under that word puts two concepts on one term. Four candidate shapes, including doing nothing, are priced in the ticket. |
| **#76 — photo capture: description field, scale reading, repeat detection (ADR-0029, `proposed`, nothing built)** | **An owner decision, and ADR-0029's stated risk is now known to be backwards.** The ADR feared a front-of-pack phrase would *fail to resolve*. Measured 2026-08-26 against the real committed index: 20 branded phrases and a 20-phrase brand-stripped control both resolve at **100%** — **the resolver never abstains**, so the failure mode is a confident WRONG match nothing downstream can detect. **Consequence: ship scale-reading, review UI and repeat detection (none depend on this); do NOT auto-apply a macro from a description** until the resolver can say "I don't know". Note `resolveItem` caps a *model*-sourced item at `confidence ≤ 0.5` while a *USDA*-sourced wrong match keeps full confidence — backwards for this failure mode. **Two ADR defects to fix when it is picked up:** it prices Gemini 2.5 Flash while the active model is `gemini-3.5-flash-lite` (every per-image cost ~1.8× low), and `analyze-photo.ts` is cited as 768px at :504 and 1920px at :124. **The cost finding that sets the order:** `dailyQuota` counts SCANS, not IMAGES, so multi-image would cost 3× against the same 3/day allowance and the `photo` `spendCeiling` is in the same wrong unit — **multi-image must not ship until the quota counts images.** Recommended order: description field → scale reading → repeat detection → multi-image, gated. **A live web lookup for brand macros is REJECTED** in the ADR; do not re-propose it. Full write-up and the measurement's own correction are on issue #76. |
...
| **`bermudezsystems.com` — the domain is the exposure, not the mailbox** | **Owner, on a diary.** Registered 2026-07-06, **expires 2027-07-06**, registrar DomainRegistry.com LLC (Northwest resells through them), registry status plain `active` with no `clientTransferProhibited`. **① Transfer: ICANN bars it within 60 days of registration, so ineligible until ~2026-09-04** — that, not Northwest's disabled buttons, was always the obstacle. From then, move to a registrar with a card on file and carry the DNS. There is ~10 months of runway. **② Recovery on the org Google account — recovery email, phone, saved 2FA backup codes — is still open, owner-only, and is the highest-value item on this list**, because it is what survives a lapse: the org Play developer account's login IS `gabriel@bermudezsystems.com`. **Do NOT delete the `google-site-verification=1EYLbXKH66lt3XNRVrp3o066X7NqEeKBhMwWO64P57U` TXT** — Play's org website verification rides on it. Mailbox is now on Microsoft 365 (verified in public DNS), so the never-configured Gmail forward is no longer a mechanism anyone depends on. One loose end: `_dmarc` still reports to Northwest's collector. Runbook: `docs/DEV_ENVIRONMENT.md` §4. |
| **Apple legal entity — the seller name** | **Nothing to do but wait.** The individual→organization migration COMPLETED 2026-08-25: the Developer account reads **Bermudez Systems LLC**, Certificates/Identifiers/Profiles is back and `eas build -p ios` works. The personal entity `86813531` is **`Deprecated`** with an empty Agreements table; the LLC `94754902` holds Paid + Free Apps Agreements Active, W-9 Active and bank `Relay (7141)` Active. The **App Store product page may still show "Gabriel Bermudez" as seller** until Apple propagates the entity — the seller name follows the legal entity, not the program membership. If it persists, that is a support question, not a missing step. Details, and the three answers that look inconsistent but are all correct, are in `CLAUDE.local.md`. |
