...
| **#101 Play quality requirements — R8 is BUILT into vc 39; Zero-Tap is vc 40** | **Nothing before the production launch** — enforcement is Feb 2027 (memory + DEX) and Apr 2027 (Zero-Tap Sign-In). **R8 was OFF in every AAB this project has ever shipped, and not by decision**: the Expo template reads `enableMinifyInReleaseBuilds ?: false` and the property was set nowhere, so coverage was 0% against a 25% floor. Now written by **step 1c** of `patch-android-release.mjs`, with 98 lines of keeps in the TRACKED `apps/mobile/android-proguard-rules.pro` installed by **step 1d** — `android/` is gitignored, so editing `proguard-rules.pro` in place loses it on the next `--clean` prebuild while the build keeps succeeding. **The measurement that decides the release plan: R8 does NOT move the fingerprint** (`ae526937…` after a clean prebuild with minify on = live vc 37), so vc 39 strands nobody and every published OTA still reaches it. **Zero-Tap Sign-In needs `androidx.credentials` 1.5.0+, a new native dep, which DOES move it** — hence vc 40, deliberately, after vc 39 is on the track. There is no Expo/RN module for `RestoreCredential`; it needs a custom Expo Module, and a runtime guard because it is Android 9+ while `minSdkVersion` here is 26. **vc 39 IS BUILT AND DEVICE-VERIFIED** (2026-08-26): `BUILD SUCCESSFUL in 6m 38s`, verifier green on all six checks, fingerprint read from the `.aab` as `ae526937…` = live vc 37, AAB **66 MB -> 59.7 MB**, 774,711 renamed entries. Exercised on the LG G6 by sideloading the R8 APK: cold launch, sign-in through Firebase Auth, Today painting real data, Trends with the fasting card, Sentry initialising — no `ClassNotFoundException` anywhere. **R8's FIRST run failed the build on a missing class, which is the good direction**, and it writes the rule it wants to `missing_rules.txt`. **Two red herrings recorded in `AGENTS.md`**: a two-minute spinner on first launch that is a cold cache and not a stripped transport, and a `PacProcessor` `NoClassDefFoundError` that is an API-28 WebView probe predating R8. **What is left before submitting: the Play Health-apps deadlock that killed vc 38** — Play refuses to release a bundle whose health permissions the declaration does not cover, and the declaration lists permissions as detected in the bundle. **And vc 39 still has to route the Play Health-apps deadlock that killed vc 38.** Bitmaps look fine on a read (`scan.tsx` holds file URIs); dynamic memory can only be read off Play Console vitals once production has traffic. |
...
| **Photo scan — CODE COMPLETE, and three things have never been seen on a device** | **Owner, with a phone — and one gap just closed.** #76 is closed and ADR-0029 is `accepted`. **MULTI-PHOTO CAPTURE IS CONFIRMED WORKING BY THE OWNER on a real meal, 2026-08-26** — that is the first end-to-end scan ever driven through the app UI on any device, and it retires the "unit-tested only" caveat this row carried all day. Automated verification was never possible: both photo pickers refuse programmatic selection, which is why it took the owner's own phone. **What the confirmation raised was a quota problem, and it is FIXED**: `dailyQuota` had counted IMAGES since that morning, so three photos of one meal spent a free user's whole day — the feature discouraged its own headline use. A scan now costs ONE slot however many images it carries (`analyzePhoto` deployed 2026-08-26, verified in PROD: a real 3-image scan reports **"2 scans left"**). **The asymmetry is deliberate and must survive:** `spendCeiling` still counts IMAGES because it is the solvency guard, `dailyQuota` counts SCANS because it is the fairness one. Measured: a 3-image scan is **1.6x** a 1-image scan, not 3x, so charging three slots overcharged by double. **A real bug was found on the way and fixed**: `dailyQuota.release` accepted a `units` argument and always refunded exactly 1, so a failed 3-image scan charged three and refunded one — silent, permanent, on the path that only runs after something already went wrong. **The cap stays at 3** and the remaining unknown is de-duplication: the verification run sent the same photo three times and got two hamburger patties, which is a pathological input but another instance of the model enumerating where it should merge. Also never seen: **repeat detection has never fired** (needs a My Foods entry matching a typed note; it is deliberately quiet, so "never suggested anything" and "broken" look identical) and **`measured` has never appeared** (needs a legible scale AND unit in one photo). **The seam not to break:** `measured` drops if `clampGrams` altered the number, so a badge never names a weight the scale did not show. **Still owner-only:** whether a matched repeat should land on an editable draft rather than logging straight to Today. |
