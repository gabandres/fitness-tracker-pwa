...
| **Play production** | **REJECTED 2026-09-03 — vc 37 was refused under the Health Connect "Minimum Scope" policy (`StepsCadence/Steps`), and the reviewer had crashed the app eleven minutes earlier (`CHANGELOG.md` 2026-09-03). Nothing is live; the store URL still 404s.** The tracks API still shows the vc 37 release on `production` as `completed` — that is the rejected submission's record, not a rollout. **vc 41 is the resubmission: BUILT and verified on 2026-09-03** (no `READ_STEPS`, the Health Connect permission delegate wired, the Android 14 privacy alias added, Zero-Tap #107 on board, fingerprint `d7ea3629…` = vc 40 so every Android OTA since 88 lands on it). **What is left, in order, and the first two are the OWNER'S because the session classifier refuses Play publishing:** (1) upload vc 41 to alpha — `node scripts/play-upload-bundle.mjs apps/mobile/android/app/build/outputs/bundle/release/app-release.aab --track alpha --commit` (or `eas submit`); (2) `node scripts/play-production-release.mjs --complete --commit --vc 41` — the track is still 128 countries / restOfWorld=false; (3) Play Console → App content → Health apps: re-open the declaration once Play has scanned vc 41, confirm `READ_STEPS` is no longer detected, remove any Steps justification, save; (4) the review. **The 128-country / no-API-for-review-status facts from the 08-29 submission still hold** (`git log -1 -- STATUS.md` before this edit has the long form) |
...
**vc 41 — the Play resubmission, carrying Zero-Tap Sign-In (#107) — is BUILT, VERIFIED, and NOT YET UPLOADED (owner step; see the Play production row).** Built 2026-09-03 from `main` at `e2412e9c` plus the rejection fixes; runtime `d7ea3629…` read from the `.aab`, identical to vc 40, so the Android OTA channel is unmoved and iOS build 60 untouched. **vc 40 was never uploaded to Play at all** (the bundles list reads 3, 33–39) and is superseded. **Device-verified on the LG VS988 with Health Connect installed from Play** (it IS installable on Android 9 — this device had simply never had it, which is why the crash was invisible here): the *Connect Health Connect* tap now launches Health Connect's permission gateway and returns, 0 `FATAL EXCEPTION`, where vc ≤ 40 died natively. **Still unexplained:** on this Android 9 sideload the gateway returns an EMPTY grant within ~100 ms after its onboarding was completed, so the app prints *Permission denied* — the crash is gone, the grant is not proven. The Android 14 alias (`patch-android-release.mjs` 4d) is the documented requirement for the reviewer's platform and cannot be exercised on this device. The evidence and the two defects fixed in vc 40 itself: the vc 40/41 rows in `apps/mobile/AGENTS.md`.
...
**The welcome intro (2026-09-02) — published on both platforms; Android reaches the test device only.** A fresh install opens on the flame catching into a hero with one sentence and a CTA, before the sign-in form (`CHANGELOG.md` 2026-09-02). Android OTA 108–109 sit on vc 40 with the rest of OTA 88+; iOS OTA 70 on build 60 reaches every App Store user. Device-verified by screen recording on the LG VS988 in light theme; **dark theme unverified** (Android 9 has no system dark mode) and iOS unverified, as standing. What it does NOT fix: the native splash still shows the OLD ring glyph and has no dark block — both are on the vc 41 remainder (§3, the push-notification row).
