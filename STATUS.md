...
| 7 | **Maintenance mode** after goal reached — looser logging, keeps the account alive through the week 8–12 fatigue. | **Merged 2026-09-05 (ADR-0037), not yet published** — see §2. The streak needed no loosening (everyone already has the 7-day freeze); what changed is the target and the reminders. |
...
| **Backup codes on the PERSONAL Google account** | **Owner-only.** The org account (`gabriel@bermudezsystems.com`) is done — 2-Step and passkeys set, **owner-reported 2026-09-05**, not read off the account. The personal Gmail (source Play account, Apple account email) still has no Backup codes and no Recovery contacts, and backup codes are what survives losing the phone. |
