...
- **The Apple Watch complication and its transport** — the watch app
  ...
  `WCSession.updateApplicationContext`. **NO LONGER "in no binary": it is on TestFlight in build 16** (`VALID`, 2026-08-03), which compiled and signed both watch products and cleared Apple's processing. Install it via the iPhone's Watch app → Available Apps. What remains unproven is everything past the compiler: the layouts at 40mm/46mm in both locales, and whether the transport actually delivers on a paired watch. What *was* verified locally: `tsc` clean, both `expo-target.config.js`
