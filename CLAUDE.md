...
- **`apps/mobile/`** — Expo SDK 57 React Native app, **live on the iOS App Store** (which version is live is a question for `STATUS.md`, not for `app.json`). Android not launched. Has its own `CLAUDE.md` → `AGENTS.md`; read those when working there. Its `main` is a custom `index.js` (not `expo-router/entry`) so the Android widget task handler registers before React mounts.
