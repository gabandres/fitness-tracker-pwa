...
- [ ] **Stripe Tax enabled.** *(still blocked — owner only)* No public API for activation. Dashboard → Settings → Tax → Get started: add legal business name, PR address, EIN (if any), auto-monitor US state thresholds, optionally register EU OSS + UK VAT, enable automatic tax calculation on all products. $0.50/tx after activation.
