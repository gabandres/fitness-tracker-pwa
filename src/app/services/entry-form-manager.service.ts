...
export class EntryFormManager {
  ...
  readonly entryDate = signal<string>(dayKeyAt(new Date(), this.store.dayBoundary()));
  ...
  onTapMeal(meal: DailyLog): void {
    ...
    this.entryDate.set(dayKeyAt(meal.date, this.store.dayBoundary()));
    this.status.set('idle');
  }
  ...
  private resetForm(): void {
    ...
    this.entryDate.set(dayKeyAt(new Date(), this.store.dayBoundary()));
