...
export class GeminiService {
  ...
  private buildWeeklyReportInstruction(
    ...
    // NB: the local `days` shape is kept (key/calories/protein/meals/...)
    // so the summary-line emission below stays unchanged.
    const days = summarizeDays(windowKeys, logs, dailyWeights).map((s) => ({
      key: s.dateKey,
      calories: s.totalCalories,
      // Per-day protein here keeps fractional precision so the avg-protein
      // line below isn't biased by 14 individual rounds.
