/**
 * U.S. Navy body-fat estimate. Pure and dependency-free (ADR-0003
 * sibling): the circumference formula and all its guards live here, so
 * this one function is the whole test surface.
 *
 ...
 *
 * Male needs `waist`, `neck`, `heightIn`; female additionally needs
 * `hip`. All in inches.
 */
export function navyBodyFat(
  ...
  if (sex === 'female') {
    ...
    pct =
      163.205 * Math.log10(girth) - 97.684 * Math.log10(heightIn) - 78.387;
  } else {
    const girth = waistIn - neckIn;
    if (girth <= 0) return null;
    pct =
      86.010 * Math.log10(girth) - 70.041 * Math.log10(heightIn) + 36.76;
  }
